...
def upgrade():
    ...
    bind = op.get_bind()
    default = json.dumps(_default_definition())
    dialect = bind.dialect.name
    if dialect == 'postgresql':
        bind.execute(sa.text('UPDATE strategies SET strategy_definition = CAST(:j AS jsonb)'), {'j': default})
    else:
        bind.execute(sa.text('UPDATE strategies SET strategy_definition = :j'), {'j': default})
