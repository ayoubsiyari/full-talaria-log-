...
export default function ImportTrades() {
  ...
  const fieldLabels = {
    ...
    entry_datetime: {
      ...
      icon: Calendar,
      description: "Trade execution date and time (required)",
      required: true,
    },
    entry_time: {
      label: "Entry Time",
      icon: Clock,
      description: "Time when the trade was opened (HH:MM:SS format)",
      required: false,
    },
    exit_time: {
      label: "Exit Time",
      icon: Clock,
      description: "Time when the trade was closed (HH:MM:SS format)",
      required: false,
    },
    pnl: {
      label: "PnL",
    ...
    strategy: {
      label: "Strategy",
      icon: Target,
      description: "Trading strategy used (e.g., trend_following, scalping)",
      required: false,
    },
    setup: {
      label: "Setup",
      icon: Settings,
      description: "Entry setup type (e.g., breakout, pullback, reversal)",
      required: false,
    },
    rr: {
      label: "Risk/Reward",
    ...
    high_price: {
      label: "High Price",
      icon: TrendingUp,
      description: "Highest price reached during the trade",
      required: false,
    },
    low_price: {
      label: "Low Price",
      icon: TrendingDown,
      description: "Lowest price reached during the trade",
      required: false,
    },
    commission: {
      label: "Commission",
      icon: DollarSign,
      description: "Trading commission/fees",
      required: false,
    },
    slippage: {
      label: "Slippage",
    ...
    notes: {
      label: "Notes",
      icon: FileText,
      description: "Trade notes or comments",
      required: false,
    },
    variables: {
      label: "Variables",
      icon: Settings,
      description: "Structured variables (e.g., setup:breakout;strategy:trend_following)",
      required: false,
    },
    var1: {
      label: "Variable 1",
  ...
  const validateMapping = () => {
    const errors = [];
    const required = ["symbol", "direction", "entry_datetime", "pnl"];
  ...
  const buildMappedTrades = () => {
    return csvData.map((row, index) => {
      ...
      const rawEntryDateTime = mapping.entry_datetime ? (row[mapping.entry_datetime] || "") : "";
      const rawEntryTime = mapping.entry_time ? (row[mapping.entry_time] || "") : "";
      const rawExitTime = mapping.exit_time ? (row[mapping.exit_time] || "") : "";
      const rawOpenTime = mapping.open_time ? (row[mapping.open_time] || "") : "";
      const rawCloseTime = mapping.close_time ? (row[mapping.close_time] || "") : "";
      ...
      const parseTradeDate = (dateStr, timeStr = null, rowIndex = 0) => {
        if (!dateStr || dateStr.toString().trim() === '') {
          // If no date provided, create a unique date based on row index
          const baseDate = new Date('2023-01-01');
          return new Date(baseDate.getTime() + (rowIndex * 24 * 60 * 60 * 1000));
        }

        let dateStrClean = dateStr.toString().trim();
        let timeStrClean = timeStr ? timeStr.toString().trim() : '';
        ...
        for (let i = 0; i < dateFormats.length; i++) {
          const match = dateStrClean.match(dateFormats[i]);
          if (match) {
            let year, month, day;
            ...
            if (i === 0) {
              ...
              // European format: DD/MM/YYYY (check if day > 12 to avoid ambiguity)
            ...
            try {
              ...
              if (timeStrClean) {
                const timeMatch = timeStrClean.match(/^(\d{1,2}):(\d{1,2})(?::(\d{1,2}))?$/);
                if (timeMatch) {
        ...
        // If all parsing attempts fail, create a unique date based on row index
        console.warn(`Could not parse date: ${dateStr}, creating unique date based on row index`);
        const baseDate = new Date('2023-01-01');
        return new Date(baseDate.getTime() + (rowIndex * 24 * 60 * 60 * 1000));
      };

      try {
        // Parse the trade date using the enhanced parser
        tradeDate = parseTradeDate(rawEntryDateTime, rawEntryTime, index);
        
        // If we have a separate entry datetime, use that instead
        if (rawEntryDateTime) {
          const entryDateTime = parseTradeDate(rawEntryDateTime, null, index);
        ...
        console.log(`Final parsed trade date:`, tradeDate, `Hour:`, hour);
        ...
        // Fallback to unique date based on row index
        const baseDate = new Date('2023-01-01');
        tradeDate = new Date(baseDate.getTime() + (index * 24 * 60 * 60 * 1000));
        entryDate = tradeDate;
        exitDate = tradeDate;
        hour = tradeDate.getHours();
      }
      ...
      const tradeObj = {
        ...
        // Dates and times - match Journal.jsx format exactly
        entry_datetime: rawEntryDateTime || (tradeDate ? tradeDate.toISOString().slice(0, 16) : ''),
        entry_date: formattedEntryDate,
        exit_date: formattedExitDate,
        entry_time: rawEntryTime || (tradeDate ? tradeDate.toTimeString().split(' ')[0] : '00:00:00'),
        exit_time: rawExitTime || (tradeDate ? tradeDate.toTimeString().split(' ')[0] : '00:00:00'),
        // For datetime-local inputs, use YYYY-MM-DDTHH:MM format (no seconds)
        open_time: rawOpenTime || (tradeDate ? tradeDate.toISOString().slice(0, 16) : ''),
        close_time: rawCloseTime || (tradeDate ? tradeDate.toISOString().slice(0, 16) : ''),
        created_at: formattedDate, // Use the parsed trade date instead of current time
        updated_at: formattedDate, // Use the parsed trade date instead of current time
        ...
        strategy: getValue('strategy') || null,
        setup: getValue('setup') || null,
        ...
        slippage: parseNumber(getValue('slippage') || 0),
  ...
  const handleSaveToDatabase = async () => {
    ...
    try {
      ...
      console.log('Sample trade created_at:', tradesToImport[0]?.created_at);
      ...
      const transformedTrades = tradesToImport.map((trade, index) => {
        const transformed = {
          ...
          ...(trade.entry_time && { entry_time: trade.entry_time }),
          ...(trade.exit_time && { exit_time: trade.exit_time }),
          // Include datetime-local fields (open_time, close_time)
          ...(trade.open_time && { open_time: trade.open_time }),
          ...(trade.close_time && { close_time: trade.close_time }),
          // Include any additional fields that might be needed
          ...(trade.entry_datetime && { entry_datetime: trade.entry_datetime }),
          // Include created_at timestamp for proper date handling
          ...(trade.created_at && { created_at: trade.created_at }),
          ...(trade.commission && { commission: parseFloat(trade.commission) }),
          ...(trade.slippage && { slippage: parseFloat(trade.slippage) }),
      ...
      const payload = {
        ...
        trades: transformedTrades,
            ...
            const res = await fetch(`${API_BASE_URL}/journal/import`, {
        method: "POST",
        headers: {
            ...
            {csvData.slice(0, 3).map((row, idx) => {
              ...
              const previewDir =
                dirTrim.startsWith("b") || dirTrim.startsWith("l")
                  ? "long"
                  : "short";
              const dateVal = row[mapping.entry_datetime] || "";
              const pnlVal =
                parseFloat(
                  row[mapping.pnl]
                    ?.toString()
      ...
      <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="mt-5 max-w-xl mx-auto text-xl text-gray-500">
            Upload your trading history to analyze your performance. Supports comprehensive data import including all trade fields, variables, and custom metrics.
          </p>
