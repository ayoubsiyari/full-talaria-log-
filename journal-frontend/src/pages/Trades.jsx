...
export default function Trades() {
  ...
  console.log('Trades: Current filters:', filters);
  ...
  useEffect(() => {
    const fetchTrades = async () => {
      ...
      try {
        // 2. Build query parameters from filters
      ...
      if (filters.variables && Object.keys(filters.variables).length > 0) queryParams.append('variables', JSON.stringify(filters.variables));
        ...
        console.log('Trades: Fetching with filters:', url);
        console.log('Trades: Query params:', queryParams.toString());
  ...
  const filteredTrades = trades
    .filter(trade => {
      const matchesSearch = trade.symbol.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           ...
                           (trade.entry_screenshot && trade.entry_screenshot.toLowerCase().includes(searchTerm.toLowerCase())) ||
                           (trade.exit_screenshot && trade.exit_screenshot.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  // Helper to determine if actions should be disabled
  const isActionDisabled = () => {
    if (!activeProfile) return true; // Disable if no profile is active
    if (activeProfile.mode === 'backtest') return false;
    if (activeProfile.mode === 'journal_live') return true;
    if (activeProfile.mode === 'journal') return true; // For now, disable edit/delete for core fields
    return false;
  };
                ...
                <table className="w-full">
                  <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <tr>
                      ...
                      <th className="text-left py-4 px-6 font-bold text-gray-700 text-sm uppercase tracking-wider">Screenshots</th>
