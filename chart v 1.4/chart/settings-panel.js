...
(function () {
    ...
    function chk(label,id,on){ return '<div class="settings-checkbox-item'+(on?' checked':'')+'" id="ck_'+id+'" style="cursor:default;"><div class="settings-checkbox">'+CSV+'</div><span class="settings-checkbox-label">'+label+'</span></div>'; }
    ...
    function clr(label,id,val){ var h=toHex(val); return '<div class="settings-input-row"><span class="settings-input-label">'+label+'</span><label style="cursor:default;display:flex;align-items:center;margin-left:auto;flex-shrink:0;position:relative;"><div id="sw_'+id+'" style="width:28px;height:28px;border-radius:6px;border:1px solid rgba(255,255,255,0.18);background:'+val+';transition:transform .12s,box-shadow .12s;" onmouseover="this.style.transform=\'scale(1.1)\';this.style.boxShadow=\'0 0 0 2px rgba(var(--sp-accent-rgb),0.45)\'" onmouseout="this.style.transform=\'scale(1)\';this.style.boxShadow=\'none\'"></div><input type="color" id="cp_'+id+'" value="'+h+'" style="position:absolute;opacity:0;pointer-events:none;width:1px;height:1px;"></label></div>'; }
    ...
    function savebtn(id,label){ return '<button id="'+id+'" style="width:100%;padding:10px;background:var(--sp-accent);color:#fff;border:none;border-radius:8px;font-weight:600;cursor:default;font-size:14px;margin-top:12px;">'+label+'</button>'; }
    function cancelbtn(id,label){ return '<button id="'+id+'" style="width:100%;padding:10px;background:rgba(var(--sp-accent-rgb),0.12);color:var(--sp-accent);border:1px solid rgba(var(--sp-accent-rgb),0.25);border-radius:8px;font-weight:600;cursor:default;font-size:14px;margin-top:6px;">'+label+'</button>'; }
    ...
    window._spPanels['general'] = {
        ...
        build: function(){
            ...
            return sec(st('Order Placement')+
                ...
                '<div class="settings-radio-item'+(mode==='instant'?' selected':'')+'" id="rad_instant" style="cursor:default;"><div class="settings-radio-circle"></div><div class="settings-radio-content"><h4>Instant</h4><p>Orders placed immediately at market price</p></div></div>'+
                '<div class="settings-radio-item'+(mode==='confirmation'?' selected':'')+'" id="rad_confirm" style="cursor:default;"><div class="settings-radio-circle"></div><div class="settings-radio-content"><h4>Confirmation</h4><p>Show a modal before each order is placed</p></div></div>'+
                '</div>')+
