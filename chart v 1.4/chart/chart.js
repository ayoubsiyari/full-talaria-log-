...
.visibility-menu__secondary {
    ...
    fitToView() {
        ...
        if (this.data.length <= visibleCandles) {
            ...
            // Position so the last candles are visible on the right
    ...
    drawAxes() {
        ...
        const lastVisibleIdxData = Math.min(maxDataLength - 1, firstVisibleIdx + (this.w - m.l - m.r) / candleSpacing);
        const lastVisibleIdx = firstVisibleIdx + (this.w - m.l - m.r) / candleSpacing; // Full visible range for labels
        
        // Calculate visible bars count for dynamic time formatting
        const visibleBarsCount = Math.ceil(lastVisibleIdxData - firstVisibleIdx);
        
        // Auto-detect timeframe from actual data if possible
        let timeframe = this.currentTimeframe || '1m';
        if (timeAxisData.length >= 2) {
            const timeDiff = timeAxisData[1].t - timeAxisData[0].t;
            const diffMinutes = timeDiff / 60000;
            if (diffMinutes >= 37000 && diffMinutes <= 50000) timeframe = '1mo';   // ~1 month
            else if (diffMinutes >= 10000 && diffMinutes <= 11000) timeframe = '1w'; // ~1 week
            else if (diffMinutes >= 1380 && diffMinutes <= 1500) timeframe = '1d';   // ~24h
            else if (diffMinutes >= 220 && diffMinutes <= 260) timeframe = '4h';     // ~4h
            else if (diffMinutes >= 55 && diffMinutes <= 65) timeframe = '1h';       // ~1h
            else if (diffMinutes >= 25 && diffMinutes <= 35) timeframe = '30m';      // ~30m
            else if (diffMinutes >= 13 && diffMinutes <= 17) timeframe = '15m';      // ~15m
            else if (diffMinutes >= 4 && diffMinutes <= 6) timeframe = '5m';         // ~5m
            else if (diffMinutes >= 0.8 && diffMinutes <= 1.2) timeframe = '1m';     // ~1m
        }
        
        // TradingView-style adaptive time intervals based on zoom level
        let labelInterval;
        if (timeframe === '1m') {
            if (visibleBarsCount > 600) labelInterval = 180;      // Every 3 hours
            else if (visibleBarsCount > 400) labelInterval = 60;  // Every 1 hour
            else if (visibleBarsCount > 200) labelInterval = 30;  // Every 30 minutes
            else if (visibleBarsCount > 100) labelInterval = 15;  // Every 15 minutes
            else if (visibleBarsCount > 40) labelInterval = 5;    // Every 5 minutes
            else labelInterval = 5;                                // Every 5 minutes
        } else if (timeframe === '5m') {
            if (visibleBarsCount > 300) labelInterval = 36;       // Every 3 hours
            else if (visibleBarsCount > 150) labelInterval = 12;  // Every 1 hour
            else if (visibleBarsCount > 75) labelInterval = 6;    // Every 30 minutes
            else labelInterval = 3;                                // Every 15 minutes
        } else if (timeframe === '15m') {
            if (visibleBarsCount > 200) labelInterval = 12;       // Every 3 hours
            else if (visibleBarsCount > 100) labelInterval = 4;   // Every 1 hour
            else labelInterval = 2;                                // Every 30 minutes
        } else if (timeframe === '30m') {
            if (visibleBarsCount > 200) labelInterval = 12;       // Every 6 hours
            else if (visibleBarsCount > 100) labelInterval = 6;   // Every 3 hours
            else if (visibleBarsCount > 50) labelInterval = 2;    // Every 1 hour
            else labelInterval = 2;                                // Every 1 hour
        } else if (timeframe === '1h') {
            // 1h TF: TradingView style intervals
            if (visibleBarsCount > 400) labelInterval = 24;       // Every day (just day numbers)
            else if (visibleBarsCount > 200) labelInterval = 12;  // Every 12 hours
            else if (visibleBarsCount > 100) labelInterval = 6;   // Every 6 hours
            else if (visibleBarsCount > 50) labelInterval = 3;    // Every 3 hours
            else labelInterval = 1;                                // Every 1 hour
        } else if (timeframe === '4h') {
            if (visibleBarsCount > 150) labelInterval = 6;        // Every 1 day
            else if (visibleBarsCount > 75) labelInterval = 3;    // Every 12 hours
            else labelInterval = 1;                                // Every 4 hours
        } else if (timeframe === '1d') {
            if (visibleBarsCount > 150) labelInterval = 30;       // Every month
            else if (visibleBarsCount > 75) labelInterval = 7;    // Every week
            else labelInterval = 1;                                // Every day
        } else if (timeframe === '1w') {
            if (visibleBarsCount > 120) labelInterval = 4;        // Every ~month
            else if (visibleBarsCount > 60) labelInterval = 2;    // Every 2 weeks
            else labelInterval = 1;                               // Every week
        } else if (timeframe === '1mo') {
            labelInterval = 1;                                    // Every month
        } else {
            labelInterval = Math.ceil(visibleBarsCount / 8);      // Default: ~8 labels
        }
        
        const minLabelSpacing = 50; // Minimum pixels between labels
        const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        
        // First pass: Find all day/month/year boundaries in visible range
        const boundaries = [];
        let prevDay = -1, prevMonth = -1, prevYear = -1;
        const scanStart = Math.max(0, Math.floor(firstVisibleIdx));
        const scanEnd = Math.min(maxDataLength - 1, Math.ceil(lastVisibleIdxData));
        
        for (let idx = scanStart; idx <= scanEnd; idx++) {
            const candle = timeAxisData[idx];
            ...
            if (prevYear !== -1 && year !== prevYear) {
                boundaries.push({ idx, type: 'year', label: String(year) });
            } else if (prevMonth !== -1 && month !== prevMonth) {
                boundaries.push({ idx, type: 'month', label: monthNames[month] });
            } else if (prevDay !== -1 && day !== prevDay) {
                boundaries.push({ idx, type: 'day', label: String(day) });
            }
        ...
        // Build set of indices that have boundary labels
        const boundaryIndices = new Set(boundaries.map(b => b.idx));
        
        // Draw all labels (boundaries + regular intervals)
        let lastLabelX = -Infinity;
        
        // Calculate timeframe interval in milliseconds
        let timeframeMs = 60000; // Default 1 minute
        if (timeAxisData.length >= 2) {
            timeframeMs = timeAxisData[1].t - timeAxisData[0].t;
        } else {
            const tfMap = {
                '1m': 60000,
                '5m': 300000,
        ...
        // Calculate label interval in milliseconds for round time alignment
        const labelIntervalMs = labelInterval * timeframeMs;
        const isCalendarBasedTf = timeframe === '1w' || /mo$/.test(String(timeframe || '').toLowerCase());
        
        // Find indices that align to round times (like :00, :30, etc.)
        const allIndices = new Set();
        
        // Scan visible data for round-time aligned candles
        const scanFrom = Math.max(0, Math.floor(firstVisibleIdx));
        const scanTo = Math.min(maxDataLength - 1, Math.ceil(lastVisibleIdx));
        for (let idx = scanFrom; idx <= scanTo; idx++) {
            const candle = timeAxisData[idx];
            if (!candle || !candle.t) continue;

            if (isCalendarBasedTf) {
                if ((idx - scanFrom) % Math.max(1, labelInterval) === 0) {
                    allIndices.add(idx);
                }
                continue;
            }
            ...
            // Check if this time aligns to a round interval
        ...
        // Fallback: if no round-time labels found, space labels evenly across visible data
        if (allIndices.size === 0 && maxDataLength > 0) {
            const step = Math.max(1, Math.floor((scanTo - scanFrom) / 5));
            for (let idx = scanFrom; idx <= scanTo; idx += step) {
                allIndices.add(idx);
            }
        }
        
        // Add extrapolated future times aligned to round intervals
        if (maxDataLength > 0 && !isCalendarBasedTf) {
            const lastCandle = timeAxisData[maxDataLength - 1];
            const lastTime = lastCandle.t;
            const lastTzDate = this.convertToTimezone(lastTime);
            const lastMinutes = lastTzDate.getHours() * 60 + lastTzDate.getMinutes();
            const intervalMinutes = labelIntervalMs / 60000;
            
            // Find next round time after last candle
            const nextRoundMinutes = Math.ceil((lastMinutes + 1) / intervalMinutes) * intervalMinutes;
            const minutesToNext = nextRoundMinutes - lastMinutes;
            const candlesToNext = Math.ceil((minutesToNext * 60000) / timeframeMs);
            
            // Add future round-time indices
            for (let futureIdx = maxDataLength - 1 + candlesToNext; futureIdx <= lastVisibleIdx; ) {
                allIndices.add(Math.round(futureIdx));
                futureIdx += labelInterval;
            }
        }
        
        boundaries.forEach(b => allIndices.add(b.idx));
        ...
        for (const idx of sortedIndices) {
            ...
            let tzDate;
            let candle = timeAxisData[idx];
            
            // For indices beyond data, extrapolate timestamp
            if (idx >= maxDataLength) {
                if (maxDataLength === 0) continue;
                const lastCandle = timeAxisData[maxDataLength - 1];
                const extrapolatedTime = lastCandle.t + (idx - maxDataLength + 1) * timeframeMs;
                tzDate = this.convertToTimezone(extrapolatedTime);
            } else {
                if (!candle || !candle.t) continue;
                tzDate = this.convertToTimezone(candle.t);
            }
            
            // Check if this is a boundary (only for actual data indices)
            const boundary = idx < maxDataLength ? boundaries.find(b => b.idx === idx) : null;
            let timeStr;
            let isSpecialLabel = false;
            
            if (boundary) {
                timeStr = boundary.label;
                isSpecialLabel = true;
            } else {
                // Regular time label - format based on timeframe
                const isDailyOrHigher = timeframeMs >= 86400000; // 1 day or more
                if (isDailyOrHigher) {
                    // For daily/weekly/monthly: show day number or date
                    const day = tzDate.getDate();
                    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                    const month = months[tzDate.getMonth()];
                    // Show "Mon D" format for extrapolated area
                    timeStr = `${month} ${day}`;
                } else {
            ...
            // Only draw if there's enough space from the last label
            // But always draw boundary labels if space allows (with smaller threshold)
            const spacing = isSpecialLabel ? minLabelSpacing * 0.7 : minLabelSpacing;
            if (x - lastLabelX >= spacing || lastLabelX === -Infinity) {
                ...
                // Draw time text - bold for special labels
                this.ctx.fillStyle = this.chartSettings.scaleTextColor;
                this.ctx.font = scaleFont;
                this.ctx.fillText(timeStr, x, this.h - 10);
