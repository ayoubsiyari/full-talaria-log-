...
(function (global) {
    ...
    function setVisibleTimeRange(chart, startSec, endSec) {
        ...
        let cwFit = desiredSpacing;
        if (typeof chart._getSpacingForCandleWidth === 'function') {
            const s1 = chart._getSpacingForCandleWidth(cwFit);
            if (s1 > 0) cwFit = cwFit * (desiredSpacing / s1);
            const s2 = chart._getSpacingForCandleWidth(cwFit);
            if (s2 > 0) cwFit = cwFit * (desiredSpacing / s2);
        }
        ...
        chart.candleWidth = Math.max(minW, Math.min(maxW, cwFit));
