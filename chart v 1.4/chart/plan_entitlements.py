"""
Subscription plan entitlements — chart API mirror of journal-backend/plan_entitlements.py.

Keep logic identical; test vectors live in shared/entitlements.json.
"""
...
def effective_backtest_limits(user, *, active_subscription=None, active_plan=None) -> dict:
...
def resolve_effective_limits_from_db(db, user, *, UserModel, SubscriptionModel, SubscriptionPlanModel):
    """Load active subscription + plan and resolve limits (resolver v2)."""
    if getattr(user, "role", "") == "admin":
        return effective_backtest_limits(user)

    sub = (
        db.query(SubscriptionModel)
        .filter(
            SubscriptionModel.user_id == user.id,
            SubscriptionModel.status.in_(list(_ACTIVE_SUB_STATUSES)),
        )
    ...
    if sub and sub.plan_id:
        plan = db.query(SubscriptionPlanModel).filter(SubscriptionPlanModel.id == sub.plan_id).first()
    return effective_backtest_limits(user, active_subscription=sub, active_plan=plan)
...
def user_should_revoke_entitlements(user, subscription_model, db_session) -> bool:
    ...
    active = (
        ...
        .filter(
            subscription_model.user_id == user.id,
