...
`;
    ...
    buildTVModal(drawing) {
        console.log('🎨 buildTVModal called for drawing.type:', drawing.type);
        console.log('🎨 drawing.constructor.name:', drawing.constructor.name);
        this.ensureTVStyles();
    ...
    buildStyleTab(container, drawing) {
        ...
        if (drawing.type === 'image-v2') {
            console.log('✅ Building image-v2 style tab!');
        ...
        if (drawing.type === 'image') {
            console.log('✅ Building image style tab!');
    ...
    buildPitchforkLevelsSection(container, drawing) {
        ...
        drawing.levels.forEach((level, idx) => {
            ...
            input.onblur = () => { input.style.borderColor = '#2a2e39'; };
            input.oninput = () => {
                const parsed = parseFloat(input.value);
                if (!isNaN(parsed)) {
                    level.value = parsed;
    ...
    buildChannelLevelsSection(container, drawing) {
        ...
        const applyChanges = () => {
            ...
            if (window.drawingManager) {
                ...
                if (actualDrawing) {
                    actualDrawing.levels = JSON.parse(JSON.stringify(drawing.levels));
                    console.log('✅ Updated actual drawing levels:', actualDrawing.levels);
                }
            }
            self.renderPreview(drawing);
        };
    ...
    setupTVModalEvents(modal, drawing) {
        ...
        modal.querySelector('.tv-btn-apply').addEventListener('click', () => {
            ...
            console.log('Drawing levels before apply:', drawing.levels);
            
            // Apply all pending changes to the drawing
            this.applyChanges(drawing);
            
            console.log('Drawing levels after applyChanges:', drawing.levels);
            ...
            if (drawingManager && drawing && drawing.type) {
                ...
                const actualDrawing = drawingManager.drawings.find(d => d.id === drawing.id);
                if (actualDrawing) {
                    console.log('Actual drawing levels before update:', actualDrawing.levels);
                    ...
                    if (drawing.levels) {
                        actualDrawing.levels = JSON.parse(JSON.stringify(drawing.levels));
                        console.log('Actual drawing levels after update:', actualDrawing.levels);
                    }
                    // Re-render and save
                    drawingManager.renderDrawing(actualDrawing);
                    drawingManager.saveDrawings();
            ...
            cancelBtn.addEventListener('click', (e) => {
                ...
                try {
                    ...
                    if (originalStyleStr && drawingManager && drawingId) {
                        ...
                        console.log('🎨 Found drawing:', !!actualDrawing, 'Original style:', originalStyle);
                        ...
                        if (actualDrawing) {
                            ...
                            console.log('🔄 Re-rendering drawing with original style');
                            drawingManager.renderDrawing(actualDrawing);
                            drawingManager.saveDrawings();
                            console.log('✅ Cancel complete - style restored');
                        }
                    } else {
        ...
        setTimeout(() => {
            console.log('🔧 Setting up checkbox listeners, found:', queryAll('.tv-checkbox').length, 'checkboxes');
            queryAll('.tv-checkbox').forEach(cb => {
                ...
                cb.addEventListener(eventType, () => {
                    ...
                    console.log('📦 Checkbox clicked:', prop, cb.dataset.tf);
                    let isChecked;
                ...
                if (prop === 'timeframe') {
                    ...
                    console.log(`👁️ Set visibility[${tf}] = ${isChecked}`);
                    ...
                    if (actualDrawing.group) {
                        ...
                        actualDrawing.group.style('display', shouldShow ? null : 'none');
                        console.log(`👁️ TF=${tf}, currentTF=${currentTf}, shouldShow=${shouldShow}`);
                    }
    ...
    applyChanges(drawing) {
        ...
        if (modal) {
            ...
            if (timeframeCheckboxes.length > 0) {
                ...
                console.log('📝 Timeframe visibility:', drawing.visibility);
            }
        }
        
        console.log('📝 Drawing style after:', drawing.style);
    ...
    applyChangesImmediately(drawing) {
        console.log('applyChangesImmediately - drawing:', drawing);
        console.log('applyChangesImmediately - pitchforkStyle:', drawing.style.pitchforkStyle);
        ...
        if (drawingManager) {
            ...
            const actualDrawing = drawingManager.drawings.find(d => d.id === drawing.id);
            console.log('Found actualDrawing:', actualDrawing);
            if (actualDrawing) {
                // Copy style properties to the actual drawing
                Object.assign(actualDrawing.style, drawing.style);
                console.log('actualDrawing.style.pitchforkStyle after assign:', actualDrawing.style.pitchforkStyle);
                // Also copy levels if they exist (for pitchfork/channel tools)
                if (drawing.levels) {
                    actualDrawing.levels = JSON.parse(JSON.stringify(drawing.levels));
                }
                console.log('Calling renderDrawing on actualDrawing');
                drawingManager.renderDrawing(actualDrawing);
            } else {
                console.log('Calling renderDrawing on drawing (no actualDrawing found)');
    ...
    cancelChanges(drawing) {
        ...
        console.log('🔄 Original style:', this.originalStyle);
        ...
        if (this.originalStyle && window.drawingManager && targetDrawing) {
            ...
            console.log('🔄 Found actual drawing:', actualDrawing);
            ...
            if (actualDrawing) {
                ...
                console.log('🔄 Restored style:', actualDrawing.style);
                
                // Re-render the drawing with restored style
                window.drawingManager.renderDrawing(actualDrawing);
                window.drawingManager.saveDrawings();
                
                console.log('🔄 Drawing re-rendered and saved');
            } else {
                console.log('🔄 ERROR: Could not find actual drawing in manager');
            }
        } else {
            console.log('🔄 ERROR: Missing originalStyle, drawingManager, or targetDrawing');
        }
...
.drawing-color-palette .drawing-color-opacity-value {
    ...
    buildPanelContent(drawing) {
        ...
        console.log('📋 showSettings called for drawing:', drawing);
        ...
        console.log('🎨 buildStyleSection called for drawing.type:', drawing.type);
        ...
        if (drawing.type === 'image-v2' || drawing.type === 'image') {
            console.log('✅ IMAGE SETTINGS MATCHED! Building upload UI...');
            // Image preview section with label
            const imageGroup = document.createElement('div');
