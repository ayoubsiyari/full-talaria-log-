...
class DrawingToolsManager {
            ...
            if (newTimeframe && newTimeframe !== lastTimeframe) {
                ...
                requestAnimationFrame(() => {
                    if (this.drawings.length > 0) {
                        console.log(`🎨 Refreshing ${this.drawings.length} drawings for new timeframe...`);
        ...
        if (canvas) {
            ...
            const onMouseDown = (event) => {
                ...
                if (event.ctrlKey && !event.shiftKey && !this.currentTool) {
                    console.log('🔲 Ctrl+mousedown detected on canvas');
            ...
            const onClick = (event) => {
                ...
                if (!event.ctrlKey && this.selectedDrawings.length > 0 && !this.isRectSelecting) {
                    console.log('🖱️ Canvas click - deselecting all');
                    this.deselectAll();
                }
            };
     ...
     */
    setTool(toolName) {
        console.log(`🎯 setTool called with: ${toolName}`);
        console.log(`🎯 Tool exists in registry:`, !!this.toolRegistry[toolName]);
        ...
        console.log(`🎨 Tool set to: ${toolName || 'none'}`);
    }
     ...
     */
    clearTool() {
        console.log('🧹 Clearing tool, current tool was:', this.currentTool);
        this.currentTool = null;
        this.drawingState.reset();
        this.svg.style('cursor', 'default');
    ...
    handleMouseDown(event) {
        ...
        console.log('🖱️ Mouse down - currentTool:', this.currentTool);
        ...
        if (!this.currentTool) {
            ...
            if (stackedLinesInfo.isStacked) {
                console.log(`📚 Stacked lines detected: ${stackedLinesInfo.count} lines from ${stackedLinesInfo.drawings.length} drawings`);
            ...
            if (stackedLinesInfo.isStacked && event.shiftKey) {
                console.log('📚 Shift+Click on stacked lines - selecting all drawings');
            ...
            if (drawingsAtPoint.length > 0) {
                if (event.altKey && drawingsAtPoint.length > 1) {
                    ...
                    if (currentlySelected) {
                        ...
                        console.log('🔄 Alt+Click: Cycling to next drawing in stack');
                    } else {
            ...
            if (drawing) {
                ...
                if (!event.shiftKey) {
                    console.log('🖱️ Clicked on empty space - deselecting all');
                    this.deselectAll();
                }
            }
            return;
        ...
        // Apply magnet snap to candle OHLC (when Ctrl/Cmd held OR magnetMode is active)
        const magnetMode = this.magnetMode || window.magnetMode || 'off';
        const isMagnetActive = magnetMode !== 'off';
        if (((event.ctrlKey || event.metaKey) || isMagnetActive) && this.currentTool) {
            console.log('🧲 Magnet mode activated');
            point = this.snapToCandle(point);
        }
        
        const toolInfo = this.toolRegistry[this.currentTool];
        console.log('🔍 Current tool:', this.currentTool, 'Tool info:', toolInfo);
        
        if (!this.drawingState.isDrawing) {
            console.log('🎬 Starting drawing for:', this.currentTool, 'Required points:', toolInfo?.points);
        ...
        console.log('✅ Point added. isComplete:', isComplete, 'tempPoints:', this.drawingState.tempPoints.length);
        
        console.log('🎯 Tool info:', this.currentTool, toolInfo);
        if (!toolInfo) {
            console.log('⚠️ No tool info found!');
            this.updateTempDrawing();
        } else if (toolInfo.dragPreview) {
            console.log('📍 Drag preview tool - tempPoints:', this.drawingState.tempPoints.length);
            if (this.drawingState.tempPoints.length === 1) {
                ...
                console.log('📍 Default points:', defaults);
                ...
                console.log('📍 Calling finalizeDrawing');
                this.finalizeDrawing();
                return;
            }
        } else {
            console.log('🎨 Updating temp drawing...');
            this.updateTempDrawing();
        }
        ...
        if (isComplete) {
            ...
            console.log('⏳ Drawing not complete yet. Need more points.');
        }
    }
    ...
    snapToCandle(point) {
        ...
        if (snapToIndicators && indicators) {
            console.log('📊 Snap to indicators enabled, checking indicators...');
            ...
            for (const ind of activeIndicators) {
                ...
                if (!indData) {
                    console.log(`📊 No data for ${ind.name}`);
                    continue;
                }
                
                console.log(`📊 Found indicator: ${ind.name}, type: ${ind.type}`);
                ...
                if (ind.type === 'bb' || ind.type === 'bollinger') {
                    ...
                    if (value !== undefined && value !== null && !isNaN(value)) {
                        ...
                        console.log(`📊 Added snap target: ${ind.name} = ${value}`);
                    }
        ...
        console.log(`🧲 Magnet snap to ${nearestName}: ${nearestPrice} (candle ${clampedIndex})`);
    ...
    handleMouseUp(event) {
        ...
        if (this.currentTool && this.isDraggingFirstTwo) {
            ...
            if (toolInfo && toolInfo.dragFirstTwo && this.drawingState.tempPoints.length === 1) {
                ...
                // Apply magnet snap
                const magnetMode = this.magnetMode || window.magnetMode || 'off';
                if (magnetMode !== 'off' || event.ctrlKey || event.metaKey) {
                    point = this.snapToCandle(point);
                }
    ...
    handleContextMenu(event) {
        ...
        if (!this.drawingState.isDrawing && this.currentTool && persistentTools.includes(this.currentTool)) {
            console.log('🖱️ Right-click deactivates persistent tool:', this.currentTool);
            this.clearTool();
        ...
        if (this.drawingState.isDrawing) {
            console.log('🖱️ Right-click cancels drawing:', this.currentTool);
            this.cancelDrawing();
            return;
        }
        ...
        if (drawingGroup) {
            ...
            if (drawing) {
                console.log('🖱️ Right-click on drawing:', drawing.type, drawing.id);
     ...
     */
    finalizeDrawing() {
        console.log('✅ finalizeDrawing called for:', this.currentTool);
        const toolInfo = this.toolRegistry[this.currentTool];
        if (!toolInfo) {
            console.log('❌ No tool info found');
            return;
        }

        console.log('✅ Creating drawing with points:', this.drawingState.tempPoints);
        ...
        if (this.currentTool === 'image' && 
            ...
            console.log('🖼️ Image tool created without image - not saving, waiting for upload');
    ...
    buildDefaultRiskReward(entry, isLong) {
        ...
        console.log('📏 Building risk/reward - stopOffset:', stopOffset, 'entry:', entry.y);
        return [
            { x: entry.x, y: entry.y },
    ...
    addDrawing(drawing) {
        ...
        console.log(`✅ Drawing added: ${drawing.type} (${drawing.id})`);
    }
    ...
    renderDrawing(drawing) {
        ...
        console.log('🔄 Rendering drawing:', drawing.type, 'with style:', drawing.style);
    ...
    setupDrawingInteraction(drawing) {
        ...
        console.log('🎯 Interactive elements found for', drawing.type, ':', interactiveElements.size(), 'elements');
        if (drawing.type === 'regression-trend') {
            console.log('🔍 Regression trend elements:', interactiveElements.nodes().map(n => ({tag: n.tagName, class: n.className.baseVal})));
        }
        ...
        const handleClick = function(event) {
            ...
            console.log('👆 CLICK on drawing:', drawing.type, 'target:', event.target.tagName, 'class:', event.target.className.baseVal || event.target.className);
            ...
            if (shapeTypes.includes(drawing.type)) {
                ...
                if (!clickedOnStroke) {
                    console.log('❌ Click was on fill, not stroke - ignoring');
                    return; // Don't select - click was on fill area
                }
            }
            
            console.log('🧹 Eraser mode check:', self.eraserMode);
            ...
            console.log('⏱️ Time since last click:', timeSinceLastClick, 'ms (last:', lastClickTime, 'now:', now, ')');
            ...
            if (timeSinceLastClick < DOUBLE_CLICK_DELAY && timeSinceLastClick > 50) {
                ...
                console.log('🖱️ Double-click detected on:', drawing.type, drawing.id);
                
                if (!drawing.locked) {
                    self.selectDrawing(drawing);
                    self.editDrawing(drawing, event.pageX, event.pageY);
                    console.log('⚙️ Opening settings for:', drawing.type);
                }
        ...
        const handleDblClick = function(event) {
            ...
            console.log('🖱️ Native dblclick on drawing:', drawing.type, drawing.id);
        ...
        handles.call(
            d3.drag()
                ...
                .on('drag', function(event) {
                    ...
                    // Apply magnet snap to candle OHLC (when Ctrl/Cmd held OR magnetMode is active)
                    const magnetMode = self.magnetMode || window.magnetMode || 'off';
                    const isMagnetActive = magnetMode !== 'off';
                    if ((event.sourceEvent.ctrlKey || event.sourceEvent.metaKey) || isMagnetActive) {
                        point = self.snapToCandle(point);
                    }
    ...
    startHandleDrag(drawing, pointIndex, event) {
        ...
        console.log(`🎯 Resizing point ${pointIndex}`);
    }
    ...
    startCustomHandleDrag(drawing, handleRole, event, pointIndex) {
        ...
        console.log(`🎯 Custom handle drag started: ${handleRole}, pointIndex: ${pointIndex}`);
    }
    ...
    handleCustomHandleDrag(event) {
        ...
        // Apply magnet snap if active
        const magnetMode = this.magnetMode || window.magnetMode || 'off';
        if (context.ctrlKey || magnetMode !== 'off') {
            context.dataPoint = this.snapToCandle(context.dataPoint);
            context.point = context.dataPoint;
        }
    ...
    endCustomHandleDrag(event) {
        ...
        console.log('✅ Custom handle resize complete');
    }
    ...
    deselectAll() {
        ...
        emptyImageTools.forEach(d => {
            console.log('🗑️ Removing empty image tool that was never uploaded');
    ...
    copyDrawing(drawing) {
        try {
            ...
            console.log('📋 Drawing copied to clipboard:', drawing.type);
        } catch (err) {
    ...
    pasteDrawing() {
        if (!this.clipboardDrawing) {
            console.log('📋 No drawing in clipboard');
            return;
        }
    ...
    toggleLock(drawing) {
        ...
        if (drawing.locked) {
            ...
            console.log('🔒 Drawing locked:', drawing.type);
        } else {
            ...
            console.log('🔓 Drawing unlocked:', drawing.type);
        }
    ...
    toggleHide(drawing) {
        ...
        if (drawing.hidden) {
            ...
            console.log('👁️ Drawing hidden');
        } else {
            if (drawing.group) {
                drawing.group.style('display', null);
            }
            console.log('👁️ Drawing shown');
        }
    ...
    deleteDrawing(drawing) {
        ...
        if (index > -1) {
                ...
                if (entryPrice) {
                    ...
                    if (orderManager.pendingOrders) {
                        ...
                        if (ordersToCancel.length > 0) {
                            console.log(`🗑️ Cancelled ${ordersToCancel.length} pending order(s)`);
                        }
                    }
                    ...
                    if (orderManager.openPositions && orderManager.chart?.svg) {
                        ...
                        positionsToRemove.forEach(order => {
                            ...
                            console.log(`🗑️ Removed order line for position #${order.id}`);
                        });
                    }
                    ...
                    if (orderManager.orderLines && orderManager.orderLines.length > 0) {
                        ...
                        linesToRemove.forEach(lineData => {
                            console.log(`🗑️ Removing pending order line #${lineData.orderId}`);
                        ...
                        svg.selectAll('.pending-order-price-text').each(function() {
                            ...
                            if (!isNaN(textPrice) && Math.abs(textPrice - entryPrice) < 0.001) {
                                ...
                                if (match) {
                                    ...
                                    console.log(`   ✓ Price matches! Removing order #${orderId}`);
                        ...
                        svg.selectAll('.pending-order-price-box').each(function() {
                            ...
                            if (match) {
                                ...
                                if (!priceText.empty()) {
                                    ...
                                    if (!isNaN(textPrice) && Math.abs(textPrice - entryPrice) < 0.001) {
                                        console.log(`   ✓ Found matching price box, removing order #${orderId}`);
                                        svg.selectAll(`.pending-${orderId}`).remove();
                                        svg.selectAll(`[class*="pending-${orderId}"]`).remove();
                                    }
                                }
            ...
            console.log(`🗑️ Drawing deleted: ${drawing.id}`);
        }
    }
    ...
    duplicateDrawing(drawing) {
        try {
            ...
            console.log('📋 Drawing cloned:', drawing.type);
        } catch (err) {
    ...
    bringToFront(drawing) {
        ...
        console.log('📐 Drawing brought to front:', drawing.type);
    }
    ...
    sendToBack(drawing) {
        ...
        console.log('📐 Drawing sent to back:', drawing.type);
    }
    ...
    saveDrawings() {
        ...
        this.drawings.forEach(d => {
            if (!d.chart) {
                d.chart = this.chart;
                console.log(`  ⚠️ Setting missing chart reference for ${d.type}`);
            }
        });
        ...
        console.log(`💾 Saved ${this.drawings.length} drawings to ${key}`);
        ...
        data.forEach((d, i) => {
            console.log(`  ${i + 1}. ${d.type}: coordinateSystem=${d.coordinateSystem}`);
        });
    }
    ...
    loadDrawings() {
        ...
        try {
            ...
            console.log(`📂 Found ${data.length} saved drawings`);
            ...
            if (isReplayActive && replaySystem.fullRawData && replaySystem.fullRawData.length > 0) {
                ...
                try {
                    ...
                    console.log(`🎮 Replay mode: Using full resampled data (${conversionData.length} candles) for loading drawings`);
                } catch (error) {
            ...
            data.forEach((item, index) => {
                ...
                if (toolInfo) {
                    ...
                    if (item.coordinateSystem === 'timestamp' && item.points) {
                        ...
                        console.log(`  Converting ${item.type} to ${this.chart.currentTimeframe}`);
            ...
            console.log(`✅ Loaded ${this.drawings.length} drawings for ${this.chart.currentTimeframe}`);
    ...
    refreshDrawingsForTimeframe() {
        ...
        console.log(`🔄 Refreshing ${this.drawings.length} drawings for timeframe ${this.chart.currentTimeframe}`);
        console.log(`📊 Chart data: ${this.chart.data.length} candles`);
        ...
        if (isReplayActive && replaySystem.fullRawData && replaySystem.fullRawData.length > 0) {
            ...
            try {
                const fullResampled = this.chart.resampleData(replaySystem.fullRawData, this.chart.currentTimeframe);
                conversionData = fullResampled;
                console.log(`🎮 Replay mode: Using full resampled data (${conversionData.length} candles) for timestamp conversion`);
            } catch (error) {
                console.warn('⚠️ Failed to resample full data in replay mode, using current data:', error);
            }
        }
        ...
        console.log(`✅ Refreshed ${this.drawings.length} drawings for ${this.chart.currentTimeframe}`);
    ...
    toggleMagnetMode() {
        ...
        console.log(`🧲 Magnet mode: ${this.magnetMode.toUpperCase()}`);
    ...
    setMagnetMode(mode) {
        if (['off', 'weak', 'strong'].includes(mode)) {
            ...
            console.log(`🧲 Magnet mode set to: ${mode.toUpperCase()}`);
            return this.magnetMode;
        }
        return this.magnetMode;
    }
    ...
    toggleKeepDrawingMode() {
        ...
        console.log(`✏️ Keep Drawing mode: ${this.keepDrawingMode ? 'ON' : 'OFF'}`);
    ...
    handleEmojiSelection(options) {
        ...
        this.setTool('emoji');
        console.log('🎨 Emoji tool activated with:', resolved);
    }
    ...
    setEraserMode(enabled) {
        ...
        console.log(`🧹 Eraser mode: ${enabled ? 'ON' : 'OFF'}`);
    }
    ...
    handleEraserClick(drawingId) {
        ...
        if (drawing) {
            this.deleteDrawing(drawing); // Pass the drawing object, not ID
            console.log(`🧹 Erased drawing: ${drawingId}`);
        }
    }
    ...
    startRectangularSelection(event) {
        ...
        const handleMouseMove = (e) => {
            if (this.isRectSelecting) {
                ...
                if (!e.ctrlKey) {
                    console.log('🔲 Ctrl released - canceling rectangular selection');
        ...
        const handleKeyUp = (e) => {
            if (e.key === 'Control' && this.isRectSelecting) {
                console.log('🔲 Ctrl key released - canceling rectangular selection');
                this.cancelRectangularSelection();
                document.removeEventListener('mousemove', handleMouseMove);
                document.removeEventListener('mouseup', handleMouseUp);
                document.removeEventListener('keyup', handleKeyUp);
        ...
        console.log('🔲 Started rectangular selection');
    }
    ...
    completeRectangularSelection() {
        ...
        console.log(`🔲 Selected ${selectedDrawings.length} drawings within rectangle`);
    ...
    cancelRectangularSelection() {
        ...
        console.log('🔲 Canceling rectangular selection');
    ...
    findDrawingsAtPoint(mouseX, mouseY) {
        ...
        for (const drawing of this.drawings) {
            ...
            if (drawing.type === 'emoji' || drawing.type === 'image') {
                console.log('🎯 Checking image tool:', drawing.type, 'at', mouseX, mouseY);
                if (typeof drawing.isPointInside === 'function') {
                    const scales = {
                        xScale: this.chart.xScale,
                        yScale: this.chart.yScale,
                    ...
                    console.log('🎯 isPointInside result:', isInside);
                    if (isInside && !hitsById.has(drawing.id)) {
                        console.log('✅ Adding image to drawingsAtPoint');
                        hitsById.set(drawing.id, { drawing, distance: 0, z });
                        continue;
                    }
                } else {
                    console.log('⚠️ No isPointInside method on drawing');
                }
            }
    ...
    findStackedLines(mouseX, mouseY, threshold = 3) {
        ...
        if (isStacked) {
            console.log(`📚 Detected ${lines.length} stacked lines at point (${mouseX.toFixed(0)}, ${mouseY.toFixed(0)})`);
        }
    ...
    saveToolStyle(toolType, style) {
        ...
        try {
            ...
            console.log(`💾 Saved style for ${toolType}`);
        } catch (e) {
    ...
    applySavedStyle(drawing) {
        ...
        if (savedStyle) {
            ...
            console.log(`📋 Applied saved style for ${drawing.type}`);
        }
    }
}
