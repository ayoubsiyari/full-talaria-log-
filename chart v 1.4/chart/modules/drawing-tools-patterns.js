...
class HeadShouldersTool extends BaseDrawing {
    ...
    render(container, scales) {
        ...
        if (pointsPx.length >= 3) {
            ...
            // TradingView-like fill: independent neckline-anchored triangles for
            // left shoulder / head / right shoulder legs.
            for (let i = 1; i < pointsPx.length - 1; i += 2) {
                const left = pointsPx[i - 1];
                const peak = pointsPx[i];
                const right = pointsPx[i + 1];
                if (!left || !peak || !right) continue;

                const leftBaseY = this._getNecklineYAtX(pointsPx, left.x);
                const rightBaseY = this._getNecklineYAtX(pointsPx, right.x);
                const peakNecklineY = this._getNecklineYAtX(pointsPx, peak.x);
                const peakY = fillAboveNeckline
                    ? Math.min(peak.y, peakNecklineY)
                    : Math.max(peak.y, peakNecklineY);

                const fillPath = `M ${left.x} ${leftBaseY} L ${peak.x} ${peakY} L ${right.x} ${rightBaseY} Z`;
