...
const LIVE_PAN_KILL = process.env.TALARIA_DISABLE_TRADE_MARKER_LIVE_PAN_GLUE_V1 === '1';
...
function installWindow() {
  global.window = {};
  if (KILL) global.window.__TALARIA_DISABLE_TRADE_MARKER_CANONICAL_PROJECTION_V1 = true;
  if (LIVE_PAN_KILL) global.window.__TALARIA_DISABLE_TRADE_MARKER_LIVE_PAN_GLUE_V1 = true;
}
...
test('pan repositions markers via chart.js::_syncOrderOverlaysDuringPan (with order line)', () => {
  ...
  if (LIVE_PAN_KILL) {
    assert.equal(Number(entryTick._state.x1), x0, 'live-pan kill: entry X stuck during pan');
    assert.equal(Number(exitTick._state.x1), ex0, 'live-pan kill: exit X stuck during pan');
    assert.equal(Number(entryTick._state.y1), y0, 'live-pan kill: entry Y stuck during pan');
  } else {
