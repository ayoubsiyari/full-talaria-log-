/**
 * TALARIA_ORDER_REGISTRY_EVICTION_V1
 * B-OREI -> C port: order-line registry eviction gate.
 *
 * Technical author of record for the gate design and fixture semantics:
 * Manager B. Manager C owns this scripts/ port and the multi-writer scan.
 *
 * ============================================================================
 ...
 * source files removes exactly the rows it disposes, under the modelled universe
 * and restricted grammar described below.
 *
 * What a GREEN result does NOT license: order lines appearing, persisting, or
 * disappearing correctly for a user; mutation grammars outside the listed
 * whole-array filter assignments; or correctness of disposal side-effects in
 * the DOM.
 *
 * This port closes B's G1 by scanning every configured source writer of the
 * shared `orderLines` registry, including both `this.orderLines = ...` and
 * `<manager>.orderLines = ...` assignment shapes. The remaining G2-G11 gaps in
 * the fixture remain in force.
 *
 * The semantic constraint is B's original one: removal-set == disposal-set.
 * Predicates are parsed and evaluated over a closed row universe; this is not
 * substring matching. The B-OREI-01 corpus still rejects the historical wrong
 * variant `(ol.isPending || true)`.
 *
 * Usage:
 *   node scripts/order-registry-eviction-invariant.mjs
 *   optional: --fixture=<path>
 *   optional: --source=<path>       single source override
 *   optional: --sources=<a,b,...>   multi-source override
 *   optional: --invert=<cellId>
 */
...
const DEFAULT_FIXTURE = ['scripts', 'fixtures', 'order-registry-eviction-sites.json'];

class GateError extends Error {}
class GrammarError extends GateError {}
class ModelError extends GateError {}
class ShapeError extends GateError {}
...
function resolveMaybeRelative(repoRoot, value) {
...
function maskInert(src) {
  ...
  let i = 0;
  let prev = '';
  const blank = (idx) => { if (idx < n && out[idx] !== '\n') out[idx] = ' '; };
...
const PUNCTUATORS = ['===', '!==', '==', '!=', '&&', '||', '??', '?.', '=>', '!', '?', ':', '.', '(', ')', '[', ']', ','];
const KEYWORD_LITERALS = new Map([['true', true], ['false', false], ['null', null], ['undefined', undefined]]);
...
function baseEnvironment(world) {
  return new Map([['this', world.self], ['orderId', ORDER_ID.target], ['ch', world.charts.ch]]);
}
...
function evaluate(node, env) {
  switch (node.t) {
    ...
    case 'this':
      ...
      return env.get('this');
    case 'ident':
      if (!env.has(node.name)) throw new ModelError(`free identifier '${node.name}' is not bound by the gate's site model`);
      return env.get(node.name);
    case 'not': return !evaluate(node.arg, env);
    case 'cond': return evaluate(node.test, env) ? evaluate(node.cons, env) : evaluate(node.alt, env);
    case 'bin': {
      ...
      switch (node.op) {
        ...
        case '!==': return left !== evaluate(node.right, env);
        case '==': return left == evaluate(node.right, env); // eslint-disable-line eqeqeq
        case '!=': return left != evaluate(node.right, env); // eslint-disable-line eqeqeq
    ...
    case 'member': {
      ...
      let key = node.name;
      if (key === undefined) key = String(evaluate(node.key, env));
...
const CONTROL_WORDS = new Set(['if', 'for', 'while', 'switch', 'catch', 'function', 'return', 'else', 'do', 'with', 'try', 'typeof', 'new', 'delete', 'void', 'case', 'in', 'of', 'await', 'yield', 'throw']);
...
function matchCloser(masked, closeIdx, open, close) {
  ...
  for (let i = closeIdx; i >= 0; i -= 1) {
    ...
    else if (masked[i] === open) { depth -= 1; if (depth === 0) return i; }
  }
  return -1;
}
...
function parsePredicateSpan(rawSource, span, where) {
  ...
  try {
    return { ...parseArrowPredicate(text), text: normalise(text) };
  } catch (error) {
    throw new GrammarError(`${where}: ${error.message} - in \`${normalise(text)}\``);
  }
}

function acceptedBasesFor(lhsExpression) {
  return new Set([lhsExpression, `(${lhsExpression}||[])`, `${lhsExpression}||[]`]);
}
...
function discoverRegistryWritesInSource(rawSource, sourceId, registryProperty) {
  const masked = maskInert(rawSource);
  if (masked.length !== rawSource.length) throw new Error('masker changed source length');
  const assignRe = new RegExp(`\\b((?:this|[A-Za-z_$][A-Za-z0-9_$]*)\\s*\\.\\s*${registryProperty})\\s*=(?!=)`, 'g');
  const writes = [];
  const perMethodCount = new Map();
  let m = assignRe.exec(masked);
  while (m !== null) {
    const lhsExpression = squash(m[1]);
    ...
    if (stmtEnd === -1) {
      ...
      try {
        record.chain = buildChain(masked, rawSource, rhsStart, stmtEnd, acceptedBasesFor(lhsExpression), 'eviction');
    ...
    if (record.kind === 'eviction') {
      const key = `${sourceId}\0${methodName}`;
      const inMethod = perMethodCount.get(key) ?? 0;
      perMethodCount.set(key, inMethod + 1);
...
function discoverRegistryWrites(sources, registryProperty) {
  const sourceModels = sources.map((source) => discoverRegistryWritesInSource(source.rawSource, source.id, registryProperty));
  const writes = sourceModels.flatMap((model) => model.writes.map((w) => ({ ...w, model })));
  const evictions = writes.filter((w) => w.kind === 'eviction');
...
function resolveDisposalLoop(masked, rawSource, methodStart, limit, arrayBinding, acceptedBases, where) {
  if (arrayBinding) {
    ...
    if (decls.length !== 1) throw new ShapeError(`${where}: expected exactly one declaration of \`${arrayBinding}\` before the eviction, found ${decls.length}`);
    ...
    if (reassignRe.test(masked.slice(declEnd, limit))) throw new ShapeError(`${where}: \`${arrayBinding}\` is reassigned between the collect-filter and the eviction`);
    ...
    if (hits.length !== 1) throw new ShapeError(`${where}: expected exactly one \`${arrayBinding}.forEach(...)\` between the collect-filter and the eviction, found ${hits.length}`);
  ...
  if (candidates.length !== 1) throw new ShapeError(`${where}: expected exactly one \`<registry filter chain>.forEach(...)\` before the eviction, found ${candidates.length}`);
...
function whereFor(site) {
  return `${site.source} ordinal ${site.ordinal} (${site.enclosingMethod}#${site.ordinalInMethod})`;
}

function verifyCollectedSetSite(site, spec, worlds) {
  ...
  const loop = resolveDisposalLoop(
    ...
    acceptedBasesFor(site.lhsExpression),
    whereFor(site),
  );
  const body = model.masked.slice(loop.callback.bodyStart, loop.callback.bodyEnd);
  const escape = ESCAPE_RE.exec(body);
  if (escape) throw new ShapeError(`${whereFor(site)}: disposal loop can skip rows - \`${escape[0]}\` appears in the \`.forEach\` body`);
  for (const world of worlds) {
    ...
    if (removed.size !== collected.size || [...removed].some((i) => !collected.has(i))) {
      throw new ShapeError(`${whereFor(site)}: removal set != disposal set in world "${world.name}" - ${setDiffReport(world, removed, collected)}`);
    }
  }
  return { shape: 'collected-set', receiver: loop.receiver };
}

function verifySingleRowSite(site, spec, worlds, meta) {
  const model = site.model;
  ...
  if (stmtEnd === -1 || between.trim() !== '') throw new ShapeError(`${whereFor(site)}: eviction is not immediately preceded by a statement`);
  ...
  if (prevStatement !== expectedCall) {
    throw new ShapeError(`${whereFor(site)}: previous statement is \`${normalise(model.masked.slice(prevStart, stmtEnd))}\`, expected \`${meta.singleRowDisposalCall}(${binding})\``);
  }
  const loop = resolveDisposalLoop(
    model.masked,
    model.rawSource,
    ...
    acceptedBasesFor(site.lhsExpression),
    `${whereFor(site)} iteration source`,
  );
  if (loop.callback.param !== binding) throw new ShapeError(`${whereFor(site)}: iteration source yields \`${loop.callback.param}\`, disposal call names \`${binding}\``);
  if (!(site.offset > loop.callback.bodyStart && site.offset < loop.callback.bodyEnd)) throw new ShapeError(`${whereFor(site)}: eviction does not sit inside the \`${loop.receiver}.forEach\` body`);
  ...
  if (!destructure) throw new ShapeError(`${whereFor(site)}: could not find \`const { ... } = ${binding}\``);
  const flag = meta.discriminatorProperty;
  const destructured = destructure[1].split(',').map((s) => s.trim().split(':')[0].trim());
  if (!destructured.includes(flag)) throw new ShapeError(`${whereFor(site)}: \`${flag}\` is not destructured from \`${binding}\``);
  ...
  if (derivedPending === null) throw new ShapeError(`${whereFor(site)}: no enclosing \`if (${flag})\` / \`else\` branch found`);
  if (derivedPending !== spec.disposal.disposedRow.pending) {
    throw new ShapeError(`${whereFor(site)}: source branch implies pending=${derivedPending}, fixture declares pending=${spec.disposal.disposedRow.pending}`);
  }
  ...
  for (const world of worlds) {
    ...
    if (disposedIndex === -1) throw new ShapeError(`${whereFor(site)}: fixture declares a disposed row outside the modelled universe`);
    const disposedRow = world.rows[disposedIndex].row;
    const iterated = keptSet(loop.chain, world, [[binding, disposedRow]]);
    if (!iterated.has(disposedIndex)) throw new ShapeError(`${whereFor(site)}: fixture disposed row is not produced by the iteration source`);
    ...
    if (removed.size !== 1 || !removed.has(disposedIndex)) {
      throw new ShapeError(`${whereFor(site)}: removal set != disposal set {${world.rows[disposedIndex].label}} in world "${world.name}" - ${setDiffReport(world, removed, expected)}`);
    }
  }
  return { shape: 'single-row' };
}

const META_KEYS = new Set(['id', 'product', 'sources', 'registryExpression', 'discriminatorProperty', 'singleRowDisposalCall', 'invariant', 'enforcedProperty', 'hazard', 'gateKind', 'gateKindReason', 'siteKey', 'knownGaps', 'wholesaleResetCount', 'notEnforced']);
const SOURCE_KEYS = new Set(['id', 'path']);
const SITE_KEYS = new Set(['ordinal', 'source', 'enclosingMethod', 'ordinalInMethod', 'disposal', 'note']);
const DISPOSAL_KEYS = {
...
function validateFixture(fixture) {
  ...
  for (const key of Object.keys(fixture)) if (key !== 'meta' && key !== 'sites') problems.push(`unknown top-level fixture key '${key}'`);
  for (const key of Object.keys(fixture.meta || {})) if (!META_KEYS.has(key)) problems.push(`unknown meta key '${key}'`);
  check(Array.isArray(fixture.meta?.sources), 'meta.sources must be an array');
  (fixture.meta?.sources || []).forEach((source, i) => {
    for (const key of Object.keys(source)) if (!SOURCE_KEYS.has(key)) problems.push(`meta.sources[${i}]: unknown key '${key}'`);
    check(typeof source.id === 'string' && source.id, `meta.sources[${i}].id must be a non-empty string`);
    check(typeof source.path === 'string' && source.path, `meta.sources[${i}].path must be a non-empty string`);
  });
  ...
  (fixture.sites || []).forEach((site, i) => {
    for (const key of Object.keys(site)) if (!SITE_KEYS.has(key)) problems.push(`site[${i}]: unknown key '${key}'`);
    check(site.ordinal === i, `site[${i}]: ordinal must equal its array index`);
    check(typeof site.source === 'string', `site[${i}]: source must be a string`);
    ...
    if (!allowed) {
      problems.push(`site[${i}]: unknown disposal.kind '${disposal.kind}' - the gate fails closed`);
      return;
    }
    for (const key of Object.keys(disposal)) if (!allowed.has(key)) problems.push(`site[${i}]: unknown disposal key '${key}' for kind '${disposal.kind}'`);
    if (disposal.kind === 'collected-set') {
      ...
      if (!row || typeof row !== 'object') {
        ...
        for (const key of Object.keys(row)) if (!DISPOSED_ROW_KEYS.has(key)) problems.push(`site[${i}]: unknown disposedRow key '${key}'`);
...
function synthCollected(collectPredicate, loopBody, evictionPredicate, lhs = 'this.orderLines') {
  return `class Probe {
    removeOrderLine(orderId) {
        const doomed = (${lhs} || []).filter(${collectPredicate});
        doomed.forEach((row) => {
${loopBody}
        });
        ${lhs} = (${lhs} || []).filter(${evictionPredicate});
    }
}
`;
}
const SYNTH_SINGLE_ROW_FIXTURE_SITE = {
  ...
  ordinalInMethod: 0,
  disposal: { kind: 'single-row', binding: 'olEntry', iterationBinding: 'lines', disposedRow: { orderId: 'target', pending: true, chart: 'ch' } },
};
...
function runSyntheticSite(source, siteSpec, meta) {
  const model = discoverRegistryWrites([{ id: 'probe', rawSource: source }], 'orderLines');
  if (model.evictions.length !== 1) return { ok: false, reason: `expected 1 eviction in the probe, discovered ${model.evictions.length}` };
  const site = model.evictions[0];
  const worlds = buildWorlds();
  try {
    if (siteSpec.disposal.kind === 'single-row') verifySingleRowSite(site, siteSpec, worlds, meta);
    else verifyCollectedSetSite(site, siteSpec, worlds);
...
const SELF_TESTS = [
  ['keyed predicate matching the disposed row is ACCEPTED', 'accept', synthSingleRow('this.orderLines = (this.orderLines || []).filter((ol) => !(ol.orderId === orderId && ol.isPending && (ol.chart || this.chart) === ch));')],
  ['identity predicate with no discriminator substring is ACCEPTED', 'accept', synthSingleRow('this.orderLines = (this.orderLines || []).filter((ol) => ol !== olEntry);')],
  ['computed access is ACCEPTED', 'accept', synthSingleRow("this.orderLines = (this.orderLines || []).filter((ol) => !(ol.orderId === orderId && ol['isPending'] && (ol.chart || this.chart) === ch));")],
  ['dead discriminator `(x.isPending || true)` is REJECTED', 'reject', synthSingleRow('this.orderLines = (this.orderLines || []).filter((ol) => !(ol.orderId === orderId && (ol.isPending || true) && (ol.chart || this.chart) === ch));')],
  ['inverted polarity is REJECTED', 'reject', synthSingleRow('this.orderLines = (this.orderLines || []).filter((ol) => !(ol.orderId === orderId && !ol.isPending && (ol.chart || this.chart) === ch));')],
  ['dropping the chart clause is REJECTED', 'reject', synthSingleRow('this.orderLines = (this.orderLines || []).filter((ol) => !(ol.orderId === orderId && ol.isPending));')],
  ['loose `==` on the id conflates the numeric twin and is REJECTED', 'reject', synthSingleRow('this.orderLines = (this.orderLines || []).filter((ol) => !(ol.orderId == orderId && ol.isPending && (ol.chart || this.chart) === ch));')],
  ['a call inside the predicate is REJECTED', 'reject', synthSingleRow('this.orderLines = (this.orderLines || []).filter((ol) => !(ol.orderId === orderId && Boolean(ol.isPending) && (ol.chart || this.chart) === ch));')],
];
const SELF_TESTS_COLLECTED = [
  ['collect-set equals removal-set is ACCEPTED', 'accept', synthCollected('(r) => r.orderId === orderId', '            if (row.orderId) { row.line?.remove; }', '(ol) => ol.orderId !== orderId')],
  ['manager-qualified assignment shape is ACCEPTED', 'accept', synthCollected('(r) => r.orderId === orderId && r.isPending', '            if (row.orderId) { row.line?.remove; }', '(ol) => !(ol.orderId === orderId && ol.isPending)', 'orderManager.orderLines')],
  ['an early `return` in the disposal loop is REJECTED', 'reject', synthCollected('(r) => r.orderId === orderId', '            if (row.isPending) return;', '(ol) => ol.orderId !== orderId')],
  ['a removal wider than the collect-set is REJECTED', 'reject', synthCollected('(r) => r.orderId === orderId && r.isPending', '            if (row.orderId) { row.line?.remove; }', '(ol) => ol.orderId !== orderId')],
];

function sourceSpecsFromFixture(repoRoot, fixture, args) {
  if (args.has('sources')) {
    const fixtureSources = fixture.meta?.sources || [];
    return args.get('sources').split(',').filter(Boolean).map((p, i) => ({
      id: fixtureSources[i]?.id || `source-${i}`,
      path: resolveMaybeRelative(repoRoot, p),
    }));
  }
  if (args.has('source')) {
    return [{ id: fixture.meta?.sources?.[0]?.id || 'source-0', path: resolveMaybeRelative(repoRoot, args.get('source')) }];
  }
  return fixture.meta.sources.map((s) => ({ id: s.id, path: resolveMaybeRelative(repoRoot, s.path) }));
}

export function runGate({ argv = process.argv.slice(2), cwd = __dirname } = {}) {
  const repoRoot = findRepoRoot(cwd);
  const args = parseArgs(argv);
  const fixturePath = args.has('fixture')
    ? resolveMaybeRelative(repoRoot, args.get('fixture'))
    : path.join(repoRoot, ...DEFAULT_FIXTURE);
  const fixture = JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
  const sourceSpecs = sourceSpecsFromFixture(repoRoot, fixture, args);
  const sources = sourceSpecs.map((source) => ({ ...source, rawSource: fs.readFileSync(source.path, 'utf8') }));
  const sourceIds = new Set(sources.map((s) => s.id));
  const schemaProblems = validateFixture(fixture);
  for (const site of fixture.sites || []) {
    if (!sourceIds.has(site.source)) schemaProblems.push(`site[${site.ordinal}]: source '${site.source}' is not listed in meta.sources / CLI sources`);
  }

  const registryProperty = String(fixture.meta?.registryExpression ?? 'this.orderLines').replace(/^(?:this|[A-Za-z_$][A-Za-z0-9_$]*)\./, '');
  const meta = {
  ...
  let modelError = null;
  try {
    model = discoverRegistryWrites(sources, registryProperty);
  } catch (error) {
    modelError = error;
    model = { sourceModels: [], writes: [], evictions: [], resets: [], unmodelled: [] };
  }
  const worlds = buildWorlds();
  ...
  const cells = [
    ...
    ['B-OREI-02', 'FIXTURE SCHEMA + MULTI-SOURCE INVENTORY COUNTS', () => {
      assert.deepEqual(schemaProblems, [], `${schemaProblems.length} fixture schema problem(s):\n  - ${schemaProblems.join('\n  - ')}`);
      if (modelError) throw modelError;
      assert.equal(model.evictions.length, expectedSites.length, `found ${model.evictions.length} filter-chain eviction(s), fixture declares ${expectedSites.length}`);
      assert.equal(model.resets.length, fixture.meta.wholesaleResetCount, `found ${model.resets.length} wholesale reset(s), fixture pins ${fixture.meta.wholesaleResetCount}`);
    }],
    ['B-OREI-03', 'SITE IDENTITY (source + enclosing method per ordinal)', () => {
      if (modelError) throw modelError;
      const n = Math.min(model.evictions.length, expectedSites.length);
      for (let i = 0; i < n; i += 1) {
        assert.equal(model.evictions[i].source, expectedSites[i].source, `ordinal ${i}: source is ${model.evictions[i].source}, fixture says ${expectedSites[i].source}`);
        ...
        assert.equal(model.evictions[i].ordinalInMethod, expectedSites[i].ordinalInMethod, `ordinal ${i}: ordinalInMethod is ${model.evictions[i].ordinalInMethod}, fixture says ${expectedSites[i].ordinalInMethod}`);
      }
      assert.equal(model.evictions.length, expectedSites.length, 'inventory length differs; see B-OREI-02');
    }],
    ['B-OREI-04', 'EVERY REGISTRY WRITE IS MODELLED (fail closed on new shapes)', () => {
      if (modelError) throw modelError;
      const failures = model.unmodelled.map((w) => `${w.source}:L${w.line} in ${w.enclosingMethod}: ${w.problem}`);
      assert.deepEqual(failures, [], `${failures.length} un-modelled write(s) to ${registryProperty}:\n  - ${failures.join('\n  - ')}`);
    }],
    ['B-OREI-05', 'SINGLE-ROW SITES: removal set == the one disposed row', () => {
      ...
      for (const spec of expectedSites) {
        ...
        if (!site) { failures.push(`ordinal ${spec.ordinal} (${spec.enclosingMethod}): site not found`); continue; }
        try { verifySingleRowSite(site, spec, worlds, meta); } catch (error) { failures.push(error.message); }
      }
    ...
    ['B-OREI-06', 'COLLECTED-SET SITES: removal set == binding-linked disposal set', () => {
      ...
      for (const spec of expectedSites) {
        ...
        const site = model.evictions[spec.ordinal];
        if (!site) { failures.push(`ordinal ${spec.ordinal} (${spec.enclosingMethod}): site not found`); continue; }
        try { verifyCollectedSetSite(site, spec, worlds); } catch (error) { failures.push(error.message); }
      }
  ...
  const results = cells.map(([id, name, runner]) => {
    let status;
    let detail;
    try {
      runner();
  ...
  const lines = [];
  lines.push('TALARIA_ORDER_REGISTRY_EVICTION_V1 order-line registry eviction gate (semantic-over-source)');
  lines.push(`Fixture: ${path.relative(repoRoot, fixturePath).split(path.sep).join('/')}`);
  lines.push(`Sources: ${sourceSpecs.map((s) => `${s.id}=${path.relative(repoRoot, s.path).split(path.sep).join('/')}`).join(', ')}`);
  lines.push(`Registry property: ${registryProperty}; property proven: removal set == disposal set`);
  lines.push('SCOPE: multi-file writer scan for whole-array filter assignments. G2-G11 remain unchanged.');
  if (invertedCell) lines.push(`Inverted cell: ${invertedCell}`);
  lines.push('');
  lines.push('Filter-chain evictions discovered (source order):');
  for (const site of model.evictions) {
    const spec = expectedSites[site.ordinal];
    lines.push(`  [${site.ordinal}] ${site.source}:L${site.line} ${site.lhsExpression} ${site.enclosingMethod}#${site.ordinalInMethod} disposal:${spec?.disposal?.kind ?? '<undeclared>'}`);
    for (const pred of site.chain.predicates) lines.push(`      keep-if ${pred.text}`);
  }
  if (model.resets.length) lines.push(`Wholesale resets NOT analysed (G3): ${model.resets.map((r) => `${r.source}:L${r.line} ${r.enclosingMethod}`).join(', ')}`);
  if (model.unmodelled.length) lines.push(`Un-modelled registry writes: ${model.unmodelled.map((r) => `${r.source}:L${r.line} ${r.enclosingMethod}`).join(', ')}`);
  lines.push('');
  ...
  if (failed > 0) lines.push(`TALARIA_ORDER_REGISTRY_EVICTION_V1 violated: ${failed} cell(s) failing`);
  return { output: lines.join('\n'), results, passed, failed, model };
}
