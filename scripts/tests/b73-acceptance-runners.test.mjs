...
test('runner contracts pin identity, claims, cleanup, and reject old layout', () => {
  ...
  assert.match(source, /subtype: verdict\.subtype/);
  assert.match(source, /passports: verdict\.passports/);
...
test('runner closure and reviewed version hashes are exact', () => {
  ...
  for (const pinName of [
    ...
    'mcPreManagerDiagnostics', 'mcPreManagerDiagnosticsTest',
  ...
  assert.deepEqual(pins.diagnosticSource.lineage,
    ['12668c87f', '98a41b602', 'd811ba5a1', 'be9e86140']);
