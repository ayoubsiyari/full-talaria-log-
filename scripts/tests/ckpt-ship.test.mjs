...
function makeHarness(t) {
  ...
  fs.copyFileSync(
    ...
    path.join(scripts, 'checkpoint-provenance.mjs'),
  );
  fs.copyFileSync(
    path.join(root, 'scripts/lib/checkpoint-provenance.mjs'),
    path.join(scripts, 'lib/checkpoint-provenance.mjs'),
  );
  fs.copyFileSync(
    path.join(root, 'scripts/fixtures/checkpoint-provenance/green-manifest.json'),
    path.join(state, '20260725b68', 'CKPT-68.provenance.json'),
  );
...
case "$*" in
  ...
  "ls-remote --tags origin refs/tags/d034-20260725b69-source refs/tags/d034-20260725b69-source^{}")
    printf '%040d\\trefs/tags/d034-20260725b69-source\\n' 1
    printf '%040d\\trefs/tags/d034-20260725b69-source^{}\\n' 0 ;;
...
`);
  for (const file of [
    path.join(bin, 'git'),
    path.join(bin, 'node'),
    path.join(scripts, 'deploy-test-checkpoint.sh'),
  ]) {
