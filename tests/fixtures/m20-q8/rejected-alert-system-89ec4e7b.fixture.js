...
class AlertSystem {
    ...
    createAlert(options) {
        ...
        // Keep the switch-OFF branch byte-for-behavior legacy: it is not
        // enclosed by the fix-only transactional epilogue.
        if (!this._m20Q8AlertCheckerIdleFixEnabled()) {
        ...
        try {
            ...
            // Synchronize from the state that actually committed, including a
            // custom collection operation that mutates and then throws.
            if (typeof this.syncAlertCheckerWithAlerts === 'function') {
                this.syncAlertCheckerWithAlerts();
            }
        }
    ...
    deleteAlert(alertId) {
        ...
        if (index > -1) {
            // Preserve the immutable parent path when the one Q8 switch is OFF.
            if (!this._m20Q8AlertCheckerIdleFixEnabled()) {
    ...
    clearAllAlerts() {
        if (confirm('Delete all alerts? This cannot be undone.')) {
            // No fix bookkeeping or try/finally is allowed onto legacy A/B.
            if (!this._m20Q8AlertCheckerIdleFixEnabled()) {
                this.alerts = [];
                this.saveAlerts();
                this.renderAlertLines();
