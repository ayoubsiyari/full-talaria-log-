...
export function isExpectedOffRedSnapshot(snapshot) {
  ...
  return iframes.length === 2 && iframes.every((panel) => {
    ...
    const duplicatedHost = panel.bars > 0 && panel.nonblack > 0
      && panel.ticker === host.ticker
      && String(panel.fileId) === String(host.fileId)
      && panel.sessionId === host.sessionId
      && panel.timeframe === host.timeframe
      && !exactPassport(panel);
    return !panel.host && !panel.errors?.length && (blank || duplicatedHost);
  });
}
