...
function fmtNum(n: number | null | undefined, opts?: { signed?: boolean }): string {
...
function MetricCard({
  ...
  accent: "default" | "green" | "red" | "cyan" | "purple";
}) {
  const ring =
    accent === "green"
...
type Props = {
  strategyId: number;
  performance: StrategyPerformanceMetrics | Record<string, unknown>;
  onClose: () => void;
};

export default function StrategyPerformanceDashboard({ strategyId, performance, onClose }: Props) {
  const p = performance as StrategyPerformanceMetrics;
        ...
        {empty ? (
          <div className="rounded-xl border border-dashed border-[var(--sl-border)] bg-[var(--sl-bg)]/50 px-6 py-12 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-xs leading-relaxed text-[var(--sl-text-muted)]">
              Tag trades with this strategy in your journal. When trades exist, win rate, P&amp;L, and profit factor
              will show here.
            </p>
          </div>
        ) : (
