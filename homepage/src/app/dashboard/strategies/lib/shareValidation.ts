 ...
 */
export function validateStrategyForShare(strategy: Record<string, unknown> | null | undefined): {
  ok: boolean;
  reason?: string;
} {
  if (!strategy) {
  ...
  const def = (strategy.strategy_definition as Record<string, unknown> | undefined) || {};
  const conditions = (def.conditions as { type?: string }[] | undefined) || [];
  ...
  const mc = (def.market_categories as string[] | undefined) || [];
  const inst = (def.instruments as string[] | undefined) || [];
  const legacy = def.instrument;
  const hasScope = mc.length > 0 || inst.length > 0 || !!(legacy && String(legacy).trim());
  if (!hasScope) {
