...
export function BacktestView() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: c.bg, fontFamily: F, overflow: "hidden" }}>
      ...
      {/* ── Original SessionsView modal (exact source) ── */}
      {iframeUrl && (
        <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(0,0,0,0.52)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ position: "relative", width: "min(980px, calc(100vw - 48px))", height: "min(690px, calc(100vh - 48px))", border: `1px solid ${c.brH}`, boxShadow: "0 20px 80px rgba(0,0,0,0.65)", background: "#05070d" }}>
            <iframe title="New Session" src={iframeUrl} style={{ width: "100%", height: "100%", border: "none" }} />
            <div onClick={closeIframe} style={{ position: "absolute", top: 10, right: 10, width: 30, height: 30, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: c.ts, background: "rgba(0,0,0,0.35)", border: `1px solid ${c.brH}` }}>×</div>
          </div>
        </div>
      )}
    </div>
