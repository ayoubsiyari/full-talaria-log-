...
export function mapApiSessionToV16(
  ...
  compositeTrades: Record<string, unknown>[]
): Record<string, unknown> {
  const cfg = sess.config;
  ...
  const trades = kpis?.trades ?? compositeTrades.length ?? 0;
  const netPnl = kpis?.net_pnl ?? compositeTrades.reduce((s, t) => s + (Number(t.pnl) || 0), 0);
  const wr = winRatePct(kpis);
  const avgRR =
    kpis?.expectancy_r != null && Number.isFinite(Number(kpis.expectancy_r))
      ? Number(kpis.expectancy_r)
      : trades > 0
        ? compositeTrades.reduce((s, t) => s + (Number(t.rMultiple) || 0), 0) / trades
        : null;
  const tradingMode =
...
export async function fetchJournalTradesForSession(sessionId: number | string): Promise<Record<string, unknown>[]> {
  try {
