...
async function main() {
  ...
  try {
    ...
    const result = await scenario.run({ browser, stack, switchOffPeerDeselect: true });
    ...
    console.log(`H-R07 switch-OFF __TALARIA_DISABLE_MULTICHART_PEER_DESELECT_V1: ${coreFail ? 'RED (expected)' : 'UNEXPECTED GREEN'}`);
  } finally {
    await browser.close();
    await stack.close();
  }
