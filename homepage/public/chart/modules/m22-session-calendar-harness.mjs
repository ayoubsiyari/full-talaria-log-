...
export const EPOCH_FLOORING_INVENTORY = [
    // ── bar-bucketing ───────────────────────────────────────────────────
    // W1/W2/W4 landed: the epoch floor for daily/weekly now lives only inside
    // `_sessionBucketStart` (and the replay helper's absent-chart fallback).
    { file: REL.chart, fn: '_sessionBucketStart (legacy fallback)', category: 'bar-bucketing',
        wiredByThisPacket: true, patch: 'W3',
        text: 'Math.floor(timestampMs / timeframeMs) * timeframeMs' },
    { file: 'chart v 1.4/chart/modules/replay-system.js', fn: '_replayBucketStart',
        category: 'bar-bucketing', wiredByThisPacket: true, status: 'LIVE',
...
export function patchFileText(rel, text) {
    const canonical = rel === REL.chartMirror ? REL.chart
        : rel === REL.pipelineMirror ? REL.pipeline
            : rel;
    let out = text;
    // Once the product carries the wiring, the find/replace pairs no longer
    // match — identity is the post-wiring text.
    if (productIsWired() && (canonical === REL.chart || canonical === REL.pipeline)) {
        return out;
    }
...
export const SHELL_GLOBS = [
    ...
    // homepage/public/chart/legacy-index.html is status=removed in module-contracts.json
