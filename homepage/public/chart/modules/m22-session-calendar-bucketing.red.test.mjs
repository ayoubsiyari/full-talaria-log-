...
const PRODUCT_WIRED = H.productIsWired();
const EFFECTIVELY_WIRED = WIRED || PRODUCT_WIRED;
...
cellTest('cellF2: crypto daily stays 00:00 UTC; crypto weekly moves to Monday 00:00 UTC', () => {
    ...
    expectEqual('F2', 'crypto1w-open-weekday-today-is-thursday-epoch-floor',
        openWeekdays.join(','), EFFECTIVELY_WIRED ? 'Mon' : 'Thu',
        `wired=${EFFECTIVELY_WIRED} opens=${openWeekdays.join(',')}`);
    ...
    expectEqual('F2', 'crypto1w-differs-from-frozen-pre-fix-iff-wired',
        H.seriesSha256(weekly) === FROZEN_TODAY.crypto1w.sha256, !EFFECTIVELY_WIRED,
...
cellTest('cellN: wired product resolves an instrument class from a product-set property', () => {
    ...
    for (const prop of H.WIRING_INTERNAL_PROPERTIES) {
        expectEqual('N', `internal-property-is-not-a-product-read:${prop}`,
            H.chartAssignmentCount(prop), (PRODUCT_WIRED && prop.startsWith('_sessionClassCache')) ? 1 : 0,
            prop.startsWith('_sessionClassCache')
                ? (PRODUCT_WIRED ? 'landed helper memo fields' : 'pre-wiring must not invent memo fields')
                : 'methods are definitions, not this.prop assignments');
    }
...
cellTest('cellM: contract sidecar is consistent with the helper and not yet in the build manifest', () => {
    ...
    expectEqual('M', 'not-yet-in-build-manifest-pre-wiring',
        manifest.modules.some((m) => m.id === 'SessionCalendar'), PRODUCT_WIRED,
        PRODUCT_WIRED ? 'SessionCalendar is in module-contracts.json' : 'must stay out until wiring lands');
...
cellTest('cellM3: the wiring is a FOUR-file change and every target accepts it', () => {
    ...
    for (const target of targets) {
        ...
        expectEqual('M3', `patch-actually-changes-file:${target.rel}`, changed, !PRODUCT_WIRED,
            PRODUCT_WIRED ? 'already landed — patch is identity' : 'pre-wiring patch must change the file');
    }
...
cellTest('cellM4: all six chart.js shells, asserted on declared load order not presence', () => {
    ...
    expectEqual('M4', 'shell-count-loading-chart-js', shells.length, 7,
        '5 script-tag shells + 2 JS-loader shells (homepage legacy-index removed)');
...
cellTest('cellM2: both resample paths route through ONE boundary implementation', () => {
    ...
    expectEqual('M2', 'scan-is-not-vacuous', scanned.length >= 19, true,
        `scanned=${scanned.length}`);
    expectEqual('M2', 'scan-covers-both-known-resample-paths',
        H.readRepo(H.REL.chart).includes('_sessionBucketStart')
        && H.readRepo(H.REL.pipeline).includes('_sessionBucketStart'), true,
        'both resample files route through _sessionBucketStart after landing');
    ...
    expectEqual('M2', 'bar-bucketing-sites-found', inCategory.length, 4);
    expectEqual('M2', 'bar-bucketing-sites-this-packet-wires', wired.length, 2,
        wired.map((s) => s.fn).join(','));
    expectEqual('M2', 'bar-bucketing-sites-left-UNWIRED',
        inCategory.filter((s) => !s.wiredByThisPacket).map((s) => s.fn).sort(),
        ['periodStart', 'resampleCandles']);
    expectEqual('M2', 'grid-coupled-boundary-computations', gridCoupled.length, 5);

    // ── replay site — wired in the same change as the resample paths ──
    //
    // `_replayBucketStart` has three callers and routes through
    // `chart._sessionBucketStart` so stepping and bars share one grid.
    ...
    expectEqual('M2', 'replay-site-routes-through-session-helper',
        replay.includes('chart._sessionBucketStart'), true);
    ...
    note('M2', 'replay-wired-with-session-calendar', true,
        'replay-system.js _replayBucketStart calls chart._sessionBucketStart for daily/weekly.');
...
test.after(() => {
    ...
    const body = {
        ...
        expectedVerdict: (STATE === 'fixed' || (STATE === 'broken' && H.productIsWired()))
            ? 'GREEN'
            : 'RED',
        buildSha: head,
