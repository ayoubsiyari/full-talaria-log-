...
function multichartChromeDomReadyV4Enabled() {
    ...
    const flagSet = (w) => {
        try {
            return !!(w && w.__TALARIA_DISABLE_MULTICHART_CHROME_DOM_READY_V4);
        } catch (_) {
            return false;
        }
    };
...
class DrawingToolsManager {
    ...
    /**
     * TradingView touch: tap = mouse click (place next point); finger move only aims the
     * blue crosshair / preview. Kill-switch:
     *   window.__TALARIA_DISABLE_TOUCH_TAP_PLACE_V1 = true
     */
    _touchTapPlaceEnabled() {
        try {
        ...
        if (!this._isPlacementModeActive() || !this._isCoarsePointerDevice()) return false;
        // Freehand tools need press-drag, not tap-tap.
        if (this.currentTool === 'brush' || this.currentTool === 'highlighter') return false;
        if (this.isDrawingPath) return false;
        return true;
    }
    ...
    _tryInheritMultichartParentArmedDrawTool() {
        ...
        // Strip stale selection chrome before arming inherited tool (b07 peer-outline regression).
    ...
    _refreshPlacementPreviewFromLastPointer(keyState = {}) {
        ...
        if (isEditing) {
            const ctrlNext = keyState.ctrlKey !== undefined ? !!keyState.ctrlKey : !!last.ctrlKey;
            const metaNext = keyState.metaKey !== undefined ? !!keyState.metaKey : !!last.metaKey;
            if (ctrlNext !== !!last.ctrlKey || metaNext !== !!last.metaKey) {
                return;
            }
        }
    ...
    _applyLiveResizeFromPointerEvent(event) {
        ...
        if (!handledByDrawing) {
            ...
            // The drawing committed the point itself (e.g. volume profile boundary).
            // Keep its timestamp anchor in sync with the new x so the end-of-drag
            // timestamp→index re-resolve does not snap the geometry back (RC-3).
            this._refreshLiveTimestampForPoint(resizeDrawing, this.resizingPointIndex);
        }
        ...
        this._syncResizeHandleChrome(resizeDrawing);
    }
    ...
    renderDrawing(drawing, opts = {}) {
        ...
        if (drawing.selected && !drawing.locked && drawing.group && !drawing.group.empty()) {
            const missingHandles = drawing.group.select('.resize-handle, .resize-handle-hit').empty();
            if (missingHandles || !liveRender) {
                this._syncResizeHandleChrome(drawing);
            }
        }
    ...
    _syncResizeHandleChrome(drawing) {
        ...
        const missing = drawing.group.select('.resize-handle, .resize-handle-hit').empty();
        if (missing) {
        ...
        if (drawing.selected && typeof drawing.select === 'function') {
            drawing.select({ skipAxisHighlights: true });
        }
        this._raiseResizeHandles(drawing);
    }
    ...
    _commitSelectedDrawingVisual(drawing, suppressToolbar = false) {
        ...
        if (!suppressToolbar && drawing.group) {
            ...
            if (isMultichartIframeEmbed() && multichartQuickbarSettingsFixEnabled()) {
                ...
                if (this.toolbar.toolbar) this.toolbar.toolbar.style.display = 'none';
            }
            notifyV9SelectionSync(this.chart, drawing);
            this._emitV9QuickBarGearReady(drawing);
        }
    }

    /** Parent V9 #tl-sett settle signal — harness waits on this, not engine #tb-settings. */
    _emitV9QuickBarGearReady(drawing) {
        if (!drawing || !multichartQuickbarSettingsFixEnabled()) return;
        if (typeof window === 'undefined') return;
        // D-024: parent shell emits after React DOM commit; skip premature parent ping.
        if (multichartChromeDomReadyV4Enabled() && isMultichartIframeEmbed()) {
            return;
        }
        let panelId = null;
        try {
        ...
        try {
            ...
            window.__talariaV9QuickBarGearReady = {
                ...detail,
                settledAt: perf && typeof perf.now === 'function' ? perf.now() : Date.now(),
            };
        } catch (_) { /* ignore */ }
        try {
            window.dispatchEvent(new CustomEvent('talaria:v9-quickbar-gear-ready', { detail }));
        } catch (_) { /* ignore */ }
        try {
            if (window.parent && window.parent !== window) {
                window.parent.dispatchEvent(new CustomEvent('talaria:v9-quickbar-gear-ready', { detail }));
            }
        } catch (_) { /* cross-origin */ }
    }
        ...
        if (addToSelection) {
            ...
            if (this.selectedDrawings.length > 1) {
                ...
                if (!suppressToolbar && lastDrawing.group) {
                    ...
                    this._emitV9QuickBarGearReady(lastDrawing);
                }
            }
        } else {
    ...
    _syncDrawingPointsFromTimestamps(drawing, options = {}) {
        ...
        const allowsExtrabar = typeof CoordinateUtils.allowsExtrabarBarIndex === 'function'
