...
const TREES = [
  { name: 'homepage', root: path.join(repoRoot, 'homepage/public/chart') },
  { name: 'chart v 1.4', root: path.join(repoRoot, 'chart v 1.4/chart') },
];
...
test('the candles themselves: where bucketing follows the zone and where it cannot', () => {
  const chartSrc = read(path.join(repoRoot, 'homepage/public/chart/chart.js'));

  // The shipped bucket rule, quoted from the product rather than assumed.
  const bucketLine = /Math\.floor\(\s*(?:candle\.)?t \/ timeframeMs\s*\) \* timeframeMs/.test(chartSrc);
  assert.ok(bucketLine,
    'RESOLVER_ABSENT_FROM_TREE: the epoch-floor bucket rule is not where this gate expects it');

  const bucket = (t, tfMs) => Math.floor(t / tfMs) * tfMs;

  // Intraday buckets that divide an hour are zone-independent for whole-hour
  // and half-hour zones alike, because every such zone's offset is a whole
  // number of minutes. The reported session is 1m, so its BARS were never the
  // defect — only their labels were.
  const oneMin = 60_000;
  ...
  // Daily and above is where the epoch floor stops agreeing with the zone: the
  // bucket boundary is UTC midnight, so a New York trader's 24 Jul daily bar
  // starts at 20:00 on 23 Jul local. This is a real limit, not a passing test.
  const day = 86_400_000;
  const nyMidnight = Date.UTC(2011, 6, 24, 4, 0, 0); // 00:00 America/New_York
  assert.notEqual(bucket(nyMidnight, day), nyMidnight,
    'if this ever passes, daily bars have started honouring a non-UTC zone and this gate is stale');
  assert.equal(bucket(nyMidnight, day), Date.UTC(2011, 6, 24, 0, 0, 0),
    'daily bars bucket on UTC midnight');

  // The session calendar that would move those boundaries exists but is not
  // wired into the live resample. Asserted so that wiring it flips this gate
  // rather than silently changing what a daily bar contains.
  const wired = /SessionCalendar|_sessionBucketStart/.test(chartSrc);
  assert.equal(wired, false,
    'session-calendar has been wired into chart.js: daily bucketing may now be zone-aware, '
    + 'so re-verify what a daily bar contains');
});
