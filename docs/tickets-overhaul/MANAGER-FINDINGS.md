...
### Post-bless S2 retest — PO results on `20260717b16` (2026-07-17)
- **PO verdict:** "complete S2 pass" EXCEPT two carve-outs.
- **CLOSED-VERIFIED (8 rows flipped, PO retest PASS S2):** H-S18, H-S20, H-S28, H-S79, H-S82 (engineering replay) + TAL-01609, TAL-01610, TAL-01626 (PENDING-DEPLOY freeze/refresh). Scoreboard now **22 / 151 = 14.6%**.
- **Carve-out 1 (known-open, NOT a regression, does not block S2):** replay follow-desync — time axis advances, candles frozen until reset-scale (▶) clicked. = H-S25 (RC3-HS25#1), planned A2 fix post-bless.
- **Carve-out 2 (H-S83 RE-OPENED — PO A/B FAIL):** mixed-TF layout with **main/focused panel = coarse (4h)** and peers = 1m/5m → PLAY runs **all** panels at 4h cadence; finest-TF unified clock (D-016) not engaging when the coarse panel is main. H-S83 held STAGED (not closed). Also holding cadence-dependent rows TAL-01600 (dual-layout play) and the re-render/jump family TAL-01629/01631/01650 pending fix + the separate 1m 10-day-jump diagnostic.
- **Dispatched Lane 2 (read-only):** `S2-COARSE-MAIN-CADENCE-diagnostic-lane2.md` — pin whether finest-TF selection scans the true global grid-min or keys off the focused/main panel's TF; A/B on `__TALARIA_MC_DISABLE_FINEST_TF_REPLAY_CADENCE`; keep separate from H-S25 follow-desync. Runs alongside the already-dispatched `REPLAY-10DAY-JUMP-diagnostic-lane2.md`.

### S1 multichart order integrity — new PO findings on `20260717b16` (2026-07-17)
- **ORD-XPNL (data-integrity, high):** multichart, 2 panels different tickers. Image evidence — GBP/USD Long recorded exit `1.31315` (a EUR/USD-range price) → PnL **-587757.04**. The order marked/closed at the **peer panel's** price = cross-ticker price bleed. This is the exact class A6-4 host-canonical order store was ratified to fix. Scoreboard row ORD-XPNL added IN-TRACK.
- **Also:** "order doesn't work good on both charts" (panel-B add/interaction intermittent, wrong-panel apply) — same ORD-MULTICHART-PARITY / A6-4 family.
- **Dispatched Lane 3 (read-only):** `ORD-MULTICHART-CROSS-TICKER-PNL-diagnostic-lane3.md` — pinpoint wrong price source, deterministic GBP+EUR repro, decide pre-A6-4 freeze-safe one-knob fix vs A6-4-required, RED spec (order records owning-panel price only). Cross-checks earlier ORD-DUP-DURATION.
- **MC-STEPFWD (replay):** multichart step-forward doesn't advance like PLAY in mixed TF/ticker → folded into Lane 2 cadence diagnostic addendum. Scoreboard row MC-STEPFWD added IN-TRACK.
