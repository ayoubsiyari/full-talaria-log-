...
| TF-REPLAY-SMOOTH | ✅ Done | **Seamless TF switch on replay multichart panels (FXReplay parity — no flash/hide/shake)** — (1) Freeze overlay rewritten from `toDataURL()`+`<img>` to a sibling `<canvas>` + `ctx.drawImage(this.canvas)` (`_captureFreezeOverlay`/`_removeFreezeOverlay`). PNG-encoding the whole canvas on every switch was slow and, under 3–4 panel replay load, could return an empty string → overlay hid → blank resized canvas showed = the "hide then show" flash. drawImage is cheap, synchronous, never taints, and a prior valid snapshot is kept on the rare failure (`_freezeOverlayValid`). (2) Replay TF switch now preserves the playhead's on-screen pixel + candle zoom instead of resetting zoom + recentering: `_captureReplayTfAnchor` (in `_beginTimeframeSwitching`) + `_restoreReplayTfAnchor` (consumed in `replay-system.js` `onTimeframeChange` host **and** follower branches, with fallback to the legacy centre-playhead / auto-scroll). Anchor cleared in `_endTimeframeSwitching`. Edited both `homepage` + `chart v 1.4` copies of `chart.js` and `modules/replay-system.js`. Network host refetch path still relies on the (now reliable) freeze overlay for no-flash; position-preserve there is a possible follow-up. |
