...
The issues flagged in **Q2–Q4** on `talaria-200-trades-full-2026.csv` are **mostly artifacts of the synthetic test generator**, not how the live chart stores journal data.

**Q1** is the important semantic question: production chart data stores **per-bar** H/L/C in R, **not** monotonic running envelopes. The test CSV looked monotonic because the generator incorrectly emitted running maxima. The dashboard path cloud applies `runningMax()` **in-app for display only**.
...
### Chart (production): **per-bar candle extremes in R**
...
The file `docs/talaria-200-trades-full-2026.csv` was produced by `chart v 1.4/chart/scripts/generate_200_trades_full_csv.py`, which **incorrectly** built running envelopes (`peak_fav = max(peak_fav, fav)`). That is a **generator bug**, not chart behavior.
...
3. **Test data:** Regenerate `talaria-200-trades-full-2026.csv` with per-bar OHLC-consistent paths before using for path-invariant QA.
...
| Test CSV outputs | `docs/talaria-200-trades-full-2026.csv`, `docs/talaria-200-trades-analytics-2026.csv` |
...
| 2026-07-01 | Regenerated `talaria-200-trades-full-2026.csv` — per-bar arrays, scalar/array alignment, signed capture_ratio |
