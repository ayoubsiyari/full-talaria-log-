...
| --- | --- | --- | --- | --- | --- |
| TAL-01696 | `OPEN` | **DEFERRED** |  | `PSL-10` | `po-eyes` in the ticket ledger. Needs a PO decision, which is what a deferral seat is for. Signature packet: `docs/plan3/LEDGER-CLOSEOUT-PACK-20260803.md` |
| TAL-01698 | `OPEN` | **DEFERRED** |  | `PSL-10` | `po-eyes`. Same packet as above |
...
| TAL-01865 | `OPEN` | **DEFERRED** |  | `PSL-11` | `owner-blocked` on A (symbol persist). **Two different things share this id** and the census flagged it as carrying two states: the *ticket* is deferred, while `TAL-01865-VIEWPORT-CONSUMER` is **KILLED** in §4c at `823e32cec`. Split here so neither reading can be quoted as the other. |
| M17-DI2 / TAL-01918 | `OPEN` | **DEFERRED** |  | `PSL-12` | `blocked-on-build`. Product guard restored on the train tip; needs stamp confirmation against the sealed build, which is verification debt rather than an unfixed defect. |
| Rayan #8 | `OPEN` | **KILLED** | — | — | **Restated on a contradiction the census caught:** this row said `OPEN` while the ticket ledger says `fixed`. The ticket ledger's own columns carry the commit and a GREEN gate, so the two sources disagreed and the stale one was here. Switch axis **UNRECORDED** — see §0. Signature packet remains `docs/plan3/LEDGER-CLOSEOUT-PACK-20260803.md` for the PO's own visual confirmation. |
...
| `E-SUS-03` | Marker defs / SVG defs | **DEFERRED** | `POST-SOAK-LEDGER-E-01` |  | Named suspect without a current E-owned KILLED/CLEARED proof shape in this ledger. Revisit after soak with MB-priced forced-GC/settle evidence or explicit owner handoff. |
| `E-SUS-04` | `_smartPrefetchCache` | **DEFERRED** | `POST-SOAK-LEDGER-E-02` |  | Real asymptotic hoard, not the monotone **36 MB/hour** V8 slope answer. Fix landed in `64b0a7a01` + `afcb87e5f`; gate `smart-prefetch-cache-discipline` is green for the static discipline row, but product freeze says it rides the post-soak build. Revisit after soak with the committed perturbation arm if needed. |
| `E-SUS-05` | V8 monotone slope | **DEFERRED** | `POST-SOAK-LEDGER-E-03` |  | Still the unresolved JS-side slope finding. Diagnostic run was contaminated/non-authoritative; perturbation rerun stays post-soak/behind C. Revisit condition: run the committed smart-cache perturbation and/or authoritative read when the box is released. |
