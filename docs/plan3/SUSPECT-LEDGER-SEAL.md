...
| `TAL-01865-VIEWPORT-CONSUMER` | B | **DEFERRED** | `PO-SIGNED: PO 2026-08-02T23:27Z` — *"Defer zoom-restore to the ledger with the mechanism recorded; 5 of 6 fields ship green"* | Any work that touches the panel boot URL or `_restorePositionToTimestamp`, or the first user report that zoom does not survive a refresh. The writer is already in place, so closing this is one consumer and a mutant — not a redesign. | **This one is live, not latent, and is recorded as an incomplete restore rather than a passing row.** Per-panel zoom is captured and persisted correctly and the boot URL now carries it, but `restoreStart` appears nowhere in `chart.js`: the param is written and read by nobody. Five of the six fields in the PO's list (symbol, timeframe, chart type, price scale, focused panel) do restore end to end. Mechanism for the fix is known: `_restorePositionToTimestamp(centerTs, candleWidth)` wants the midpoint of the stored window plus the stored `candleWidth`. Deferred on blast radius — it runs on every chart boot, in the engine, on seal night. |

> Deferral note on `TAL-01865-VIEWPORT-CONSUMER`: this row is deliberately phrased as a **user-visible gap that remains open**, not as a hazard. Zoom will not survive a refresh in the sealed build. It sits in 4b rather than section 4 because the decision was taken on the record with a mechanism attached, but nobody should read this table and conclude the restore manifest is complete.
