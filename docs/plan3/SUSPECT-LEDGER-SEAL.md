...
| --- | --- | --- | --- | --- | --- |
| 1 | **SHELL-PLAY-01** — V9 shell play override | **CLEARED** | — | ~~`PSL-26`~~ seat released | **CLEARED at the shipped envelope, 11:0x+01:00.** Measured by B on b126, `--speed=10 --step=60`, artifact `.scratch/b-step60-shellplay.json`, **23/23 `PUBLISH_CORRECT`**: *"play() as installed started every realm it was asked to — no realm needed the prototype fallback"*, all four realms playing, playhead advancing **606–608 market-s/wall-s measured independently of the meter** over 60.44 s in six slices, read-back **597.5** against an expected 600 (ratio 0.996). The override sets `__shellPlayOverrideInert = !(isPlaying || isPlayStarting)`, and both host `play()` calls returned `isPlayStartingOnReturn: true` — so **the flag computes `false`. The override is not inert.** The row said *present, mirrored, bound to nothing*; at the envelope the PO will test, it is bound and it works. **What is NOT proved, and it is the interesting half.** Every earlier INERT reading was taken at `--step=1`, where the product delivers **0.08 market-s/wall-s** — one market-second per twelve — but I could not run the comparison arm to confirm the step *caused* the verdict: the instrument now **refuses** `--step=1` with `HARNESS_FAILED`, *"below the shipped PO/W1 envelope (60s native-bar floor)"*. So the causal claim rests on inference, not on a controlled pair, and the inference is weaker than it looks — the inert flag is computed from `isPlaying || isPlayStarting`, **not** from playhead motion, so why step 1 would flip it is unexplained. What is settled is narrower and sufficient: **at the shipped envelope the play path is not the defect**, so the four eliminated exits and the missing mechanism were a hunt in the wrong path. The residue belongs to `PSL-38` (D), the 1s-step product rate.
...
| `B-SUS-28` | `SHELL-PLAY-01` — ~~present, mirrored, bound to nothing~~ | **CLEARED** | — | seat released | **CLEARED at the shipped envelope, 11:0x+01:00.** Measured by B on b126, `--speed=10 --step=60`, artifact `.scratch/b-step60-shellplay.json`, **23/23 `PUBLISH_CORRECT`**: *"play() as installed started every realm it was asked to — no realm needed the prototype fallback"*, all four realms playing, playhead advancing **606–608 market-s/wall-s measured independently of the meter** over 60.44 s in six slices, read-back **597.5** against an expected 600 (ratio 0.996). The override sets `__shellPlayOverrideInert = !(isPlaying || isPlayStarting)`, and both host `play()` calls returned `isPlayStartingOnReturn: true` — so **the flag computes `false`. The override is not inert.** The row said *present, mirrored, bound to nothing*; at the envelope the PO will test, it is bound and it works. Three attempts to fix a play path that was never broken at the envelope is the cost of the wrong diagnosis, and the Director found it, not me: my inertness observations were **all** taken at the old `--step=1` default. **What is NOT proved, and it is the interesting half.** Every earlier INERT reading was taken at `--step=1`, where the product delivers **0.08 market-s/wall-s** — one market-second per twelve — but I could not run the comparison arm to confirm the step *caused* the verdict: the instrument now **refuses** `--step=1` with `HARNESS_FAILED`, *"below the shipped PO/W1 envelope (60s native-bar floor)"*. So the causal claim rests on inference, not on a controlled pair, and the inference is weaker than it looks — the inert flag is computed from `isPlaying || isPlayStarting`, **not** from playhead motion, so why step 1 would flip it is unexplained. What is settled is narrower and sufficient: **at the shipped envelope the play path is not the defect**, so the four eliminated exits and the missing mechanism were a hunt in the wrong path. The residue belongs to `PSL-38` (D), the 1s-step product rate.
