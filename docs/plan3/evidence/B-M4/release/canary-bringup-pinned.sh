...
#
# TO GRADE A PIN, DO NOT USE THIS SCRIPT — use canary-grade-lane.sh, which runs
# the pin as a second container on 127.0.0.1:3001 and leaves live alone. On
# 2026-07-29 this script displaced b99 with b85 three times while the PO was
# measuring, because the grading handoff pointed here. It now refuses unless the
# caller means to move the wire:
#
#   ALLOW_LIVE_DISPLACEMENT=1 CHART_BUILD_ID=<id> bash canary-bringup-pinned.sh
set -euo pipefail
...
if [[ "${ALLOW_LIVE_DISPLACEMENT:-0}" != "1" ]]; then
  cat >&2 <<'GUARD'
REFUSING: this replaces the LIVE canary stack, which is what the PO measures.

To grade a pinned build, use the grade lane instead — it runs the pin beside
live, not over it:

  cd /opt/talaria
  CHART_BUILD_ID=<id> bash canary-grade-lane.sh up
  bash canary-grade-lane.sh status
  bash canary-grade-lane.sh down

See HANDOFF-C-GRADE-LANE-20260729.md.

If you really do intend to move the live wire (rollback, restoring a floor),
re-run with ALLOW_LIVE_DISPLACEMENT=1 and say so in your journal.
GUARD
  exit 2
fi
...
# Password auth when a pass is supplied, key auth otherwise. This script is the
# rollback path, so it must not be unusable just because the pass env is absent —
# that happened on 2026-07-29 when model routing was restored and the env went
# with it, leaving no way to move the wire back.
if [[ -n "${TALARIA_TEST_HOST_PASS:-}" ]]; then
  ASK="$(mktemp)"
  cleanup() { rm -f "$ASK"; }
  trap cleanup EXIT
...
elif ssh -p "$PORT" -o BatchMode=yes -o ConnectTimeout=15 \
       -o StrictHostKeyChecking=accept-new "root@${HOST}" true 2>/dev/null; then
  echo "no TALARIA_TEST_HOST_PASS — using key auth" >&2
  SSH=(ssh -p "$PORT" -o BatchMode=yes -o ConnectTimeout=20 -o StrictHostKeyChecking=accept-new)
else
  echo "no TALARIA_TEST_HOST_PASS(_B64) and key auth to root@${HOST}:${PORT} failed" >&2
  exit 1
fi
