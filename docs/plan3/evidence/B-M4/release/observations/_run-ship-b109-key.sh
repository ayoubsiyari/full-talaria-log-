...
echo "=== 9. B-0199 payload: A's CPU cuts + countdown guard on the wire ==="
"${SSH[@]}" "root@${HOST}" "set -e
H=/usr/share/nginx/html/chart
docker exec talaria-homepage-1 sh -c \"
  printf 'raf_coalesce_switch=%s countdown_guard_switch=%s legend_css_switch=%s\n' \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_RAF_PAINT_COALESCE_V1' \$H/chart.js)\\\" \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_COUNTDOWN_NULL_GUARD_V1' \$H/chart.js)\\\" \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_IND_LEGEND_CSS_IDEMPOTENT_V1' \$H/modules/indicator-ui.js)\\\"
\"
"

echo "=== 8. B-0198 payload: A's XFRAME-REF-RELEASE on the wire ==="
"${SSH[@]}" "root@${HOST}" "set -e
H=/usr/share/nginx/html/chart
docker exec talaria-homepage-1 sh -c \"
  F=\$H/multichart-prod/multichart-manager.js
  printf 'xframe_switch=%s xframe_release_fn=%s stash_handles=%s about_blank_absent=%s\n' \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_MC_XFRAME_REF_RELEASE_V1' \\\$F)\\\" \\
    \\\"\\\$(grep -c 'mcXframeRefRelease\|_releaseCrossFrameRefs' \\\$F)\\\" \\
    \\\"\\\$(grep -c 'mcStashPanelHandles' \\\$F)\\\" \\
    \\\"\\\$(grep -c \\\"src = 'about:blank'\\\" \\\$F)\\\"
\"
"
