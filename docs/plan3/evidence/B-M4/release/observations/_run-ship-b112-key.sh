...
echo "=== 6. B-0202 payload: the four A packets and D's lane, on the wire ==="
"${SSH[@]}" "root@${HOST}" "set -e
H=/usr/share/nginx/html/chart
docker exec talaria-homepage-1 sh -c \"
  printf 'tf_anchor=%s tf_heal=%s countdown_guard=%s\n' \\
    \\\"\\\$(grep -c 'TF-DOWNSHIFT-ANCHOR\|_tfDownshiftAnchor\|tfDownshiftAnchor' \$H/chart.js)\\\" \\
    \\\"\\\$(grep -c 'TF_DOWNSHIFT\|tfDownshiftEmptyRecovery\|_tfDownshiftHeal' \$H/chart.js)\\\" \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_COUNTDOWN_NULL_GUARD_V1' \$H/chart.js)\\\"
  printf 'clone_cut_switch=%s reseed_switch=%s labelhandle_switch=%s\n' \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_MC_INCREMENTAL_RAWDATA_COPY_V1' \$H/chart.js)\\\" \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_REPLAY_RESEED_INCREMENTAL_V1' \$H/chart.js)\\\" \\
    \\\"\\\$(grep -c '__TALARIA_DISABLE_LABEL_HANDLE_WIPE_V1' \$H/modules/drawing-tools-shapes.js)\\\"
  printf 'D_pending_clear=%s D_m24_restore=%s\n' \\
    \\\"\\\$(grep -c '_emitPendingMirrorSync' \$H/modules/order-manager.js)\\\" \\
    \\\"\\\$(grep -c 'displayedTradeId\|_restoreDisplayedId\|restoreIdentity' \$H/modules/order-manager.js)\\\"
\"
echo '--- served order-manager matches the canonical size (mirror parity on the wire)'
docker exec talaria-homepage-1 sh -c \"wc -l \$H/modules/order-manager.js\"
echo '--- b110/b111 image cuts still in place'
docker exec talaria-homepage-1 sh -c \"
  printf 'loader_brand_declared=%s dead_getter=%s\n' \\
    \\\"\\\$(grep -c 'width=\\\"880\\\" height=\\\"822\\\"' \$H/dist-v9/index.html)\\\" \\
    \\\"\\\$(grep -c 'async getBrandLogoImage' \$H/modules/screenshot-manager.js)\\\"
\"
python3 - <<'PY'
import struct, subprocess
b = subprocess.run(['curl','-sS','http://127.0.0.1:3000/chart/modules/logo-04.png'], capture_output=True).stdout
w, h = struct.unpack('>II', b[16:24])
print('loader_brand=%dx%d decoded=%.2fMB' % (w, h, w*h*4/1048576))
assert max(w,h) <= 880
print('IMAGE_CUTS_INTACT=yes')
PY
echo '--- earlier payloads (b104-b109)'
docker exec talaria-homepage-1 sh -c \"
  printf 'stash_handles=%s xframe=%s claim_split=%s ledger=%s raf=%s\n' \\
    ...
    \\\"\\\$(grep -c '__TALARIA_DISABLE_RAF_PAINT_COALESCE_V1' \$H/chart.js)\\\"
\"
"
