...
echo "=== 6. B-0201 payload: every brand asset measured over HTTP ==="
"${SSH[@]}" "root@${HOST}" "set -e
python3 - <<'PY'
import struct, subprocess, sys
# Target max edge per asset, from scripts/lib/asset-decoded-budget.mjs.
targets = {
    'modules/logo-04.png': 880,
    'modules/logo-05.png': 600,
    'modules/logo-06.png': 600,
    'modules/logo-08.png': 256,
    'modules/logo-09.png': 256,
    'modules/logo-14.png': 600,
}
total = 0
bad = []
for rel, maxedge in sorted(targets.items()):
    url = 'http://127.0.0.1:3000/chart/' + rel
    r = subprocess.run(['curl','-sS','-H','Cache-Control: no-cache',url], capture_output=True)
    b = r.stdout
    if len(b) < 24 or b[:4] != b'\x89PNG':
        bad.append('%s did not serve a PNG (%d bytes)' % (rel, len(b)))
        continue
    w, h = struct.unpack('>II', b[16:24])
    dec = w*h*4
    total += dec
    flag = 'OK ' if max(w,h) <= maxedge else 'OVER'
    if max(w,h) > maxedge:
        bad.append('%s is %dx%d, target %d' % (rel, w, h, maxedge))
    print('%s %-22s %5dx%-5d disk=%6dB decoded=%7.2fMB target_max_edge=%d' % (flag, rel, w, h, len(b), dec/1048576, maxedge))
print('SERVED_BRAND_DECODED_TOTAL_MB=%.2f (was 251.35 across all copies, 172.98 for these six)' % (total/1048576))
if bad:
    print('BRAND_ASSET_FAILURES=' + '; '.join(bad)); sys.exit(1)
print('ALL_BRAND_ASSETS_WITHIN_TARGET=yes')
PY
"

echo "=== 7. the dead brand cache is gone from the served module ==="
"${SSH[@]}" "root@${HOST}" "set -e
H=/usr/share/nginx/html/chart
docker exec talaria-homepage-1 sh -c \"
  F=\$H/modules/screenshot-manager.js
  printf 'on_demand_loader=%s dead_getter_in_code=%s session_field_in_code=%s\n' \\
    \\\"\\\$(grep -c 'loadBrandLogoForExport' \\\$F)\\\" \\
    \\\"\\\$(grep -c 'async getBrandLogoImage' \\\$F)\\\" \\
    \\\"\\\$(grep -c 'this._brandLogoImage' \\\$F)\\\"
\"
echo '--- the loader brand declares its intrinsic size in the served shell'
docker exec talaria-homepage-1 sh -c \"grep -oE '<img class=\\\"loader-brand\\\"[^>]*>' \$H/dist-v9/index.html\"

echo '--- prior payloads still on the wire (b104-b110 regression check)'
docker exec talaria-homepage-1 sh -c \"
