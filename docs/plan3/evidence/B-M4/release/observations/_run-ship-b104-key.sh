...
# The PO's heap run owns :3000 until /root/talaria-restore/MEASUREMENT-IN-PROGRESS
# is cleared. Ship aborts rather than displacing a live measurement.
...
# M17-DI2 guard + prior cuts must be on the wire, not just in the tree
docker exec talaria-homepage-1 sh -c 'grep -c __TALARIA_DISABLE_COMPLETED_BAR_CLOSE_GUARD_V1 /usr/share/nginx/html/chart/chart.js /usr/share/nginx/html/chart/modules/replay-system.js /usr/share/nginx/html/chart/multichart-prod/panel-cmd-bridge.js; grep -c __TALARIA_DISABLE_MC_RELEASE_BLOB_WORKER_V1 /usr/share/nginx/html/chart/multichart-prod/multichart-manager.js' || true
"
