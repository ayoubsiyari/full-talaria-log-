...
|---|---|---|---|
| MODULE-CONTRACT-PREFLIGHT-V1 | `TALARIA_MODULE_CONTRACT_PREFLIGHT_V1` | `scripts/module-contract-preflight.mjs`, `scripts/module-contracts.json` | LIVE — W63d GATE-01: inventory includes `/chart/multichart/chart-host.html` (source+public); missing ModulePresenceRuntime + IndicatorPerf RED is collected independently of build-stamp RED, path/engine-load evidence prevents harness/servable relabels from dropping contracts, and pinned real `dist-v9` mutants prove `nomodule`, nested inert wrappers, false-literal controls, and never-called function/arrow loaders do not satisfy presence |
