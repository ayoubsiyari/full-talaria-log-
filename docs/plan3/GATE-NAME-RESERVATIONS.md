...
| M6-SCHEDULER-CENSUS-INSTRUMENTED | baseline and final scheduler census both have `installedWindows>=1`, `windowCount>=1`, no wrapper errors, observed `timerCallbacks` or `rafCallbacks` growth above the pinned margin, and unchanged `installedAt` epoch per window label; blind/all-zero, reset/reinstall, or displaced-wrapper census is RED and cannot count as PO defect reproduced | LIVE (W57e hardened) |
