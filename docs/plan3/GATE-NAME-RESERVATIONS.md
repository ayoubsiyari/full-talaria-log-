...
|---|---|---|---|
| MODULE-CONTRACT-PREFLIGHT-V1 | `TALARIA_MODULE_CONTRACT_PREFLIGHT_V1` | `scripts/module-contract-preflight.mjs`, `scripts/module-contracts.json` | LIVE — W63c GATE-01: inventory includes `/chart/multichart/chart-host.html` (source+public); missing ModulePresenceRuntime + IndicatorPerf RED is collected independently of build-stamp RED, path/engine-load evidence prevents surface relabels from dropping contracts, and inert/dead loader literals cannot satisfy presence |
...
| NC-M6-WRAPPER-DISPLACED-BROADCASTCHANNEL | unit/fault probe: displaced `BroadcastChannel` wrapper makes `M6-SCHEDULER-CENSUS-INSTRUMENTED` RED and cannot mint FIXED GREEN | LIVE (W57f) |
