...
## CURRENT STATE — A's lane · maintained in place · last updated 11:37+01:00

<!-- STATE-BLOCK-FRESHNESS entriesBelow=274 -->
...
| **W1's 0.08 was the HARNESS, not the product** | **PO measured 600 bars/min by hand at speed 10, step 1m — exactly governed** | `PO_HAND_MEASUREMENT` supersedes my reading as a statement about the product. My 0.08 market-s/wall-s is real *as a measurement of my apparatus* and says nothing about delivery. **Do not quote 0.08 as a product rate.** Harness play path and step validity are D's row |
| W1 apparatus reading, sealed b126 | 0.08 market-s/wall-s | `MEASURED_PLAYHEAD_DELTA_THROUGH_A_DEFECTIVE_HARNESS` — all four realms, 5 market-s over 60.01 s in 6 × 10 s slices, clean tree, focus pinned and recorded. `order01b-readback-canary.json`. Valid only as evidence about the harness |
| sustained 4-panel delivery, step 1 s | 0.07 market-s/wall-s | `MEASURED_PLAYHEAD_DELTA_THROUGH_A_DEFECTIVE_HARNESS` — 4 market-s over 60.01 s in 6 slices. Agrees with W1's 0.08, and both agree about the *harness*. Neither is a product figure |
| **GOVERNOR-REF-01 launch pre-flight** | **live, refuses before arming** | `CELLS_AND_MUTANTS` — 30 cells, 23/23 mutants killed, 10 of them targeting this gate. Refuses on `REFERENCE_COARSER_THAN_MIN_DISPLAYED`, `FOCUSED_PANEL_COARSER_THAN_MIN_DISPLAYED`, `REFERENCE_UNREADABLE`, `PROBE_ABSENT`, `NO_REALMS_READ`. **Not yet exercised against a live mixed-timeframe layout** — no run since the freeze |
| **GOVERNOR-REF-01 focus invariant** | **held across the whole run, not the window** | `CELLS_AND_MUTANTS` — sampled at pre-flight, before the window and after it; `FOCUS_MOVED_DURING_RUN` is its own verdict state, ranked above `PUBLISH_WRONG`. States: `FOCUS_HELD`, `FOCUS_MOVED_DURING_RUN`, `FOCUS_UNREADABLE`, `FOCUS_NOT_SAMPLED`. The window-only version I shipped first is now a killed mutant |
| **CENSUS-TREE-01** | **the gate census cannot execute in the working tree** | `REFUSAL_EXERCISED` — 4 refusal states verified live; `--scan-only` still legal read-only, a scratch copy outside the repo verified executing there with the artifact still landing in the real repo. `PSL-42`, hard not advisory |
...
| gates resolving root by fixed depth | 121 | `MEASURED_POPULATION_PRE_FIX` — the count *before* the fix landed at `f73e7ca00` / `0888d6757`. 92 files were repaired the same morning, so this is history, not current state |
| mirrored gate pairs broken by depth | 26 of 29, 14 never ran at all | `MEASURED_PRE_FIX` — same caveat. Six further never-run gates repaired and parked at `30262c421`; the re-census that would give a current figure is **frozen**, so there is no current number and I am not implying one |
| host-scope adoption | **52/53 via the shared loader, 0 holes** | `HOST_SCOPE_TOTAL` + `STATIC_ONLY_SOURCE_GATE` — counts how files obtain puppeteer, not that every launch path is wrapped; the runtime guarantee is the launch binding, proven by an end-to-end cell (exit 3, browser never reached) |
| ~~the reference and scaling bands~~ | **withdrawn 22:20+01:00** | `HOST_SHARED_DURING_RUN` — all twelve arms ran inside E's V8 read; see "Not quotable". Re-take under host scope pending |
...
- **The 20:55+01:00 reference pass, in either direction.** TradingView 779.99 MB against our 564.3
  at one chart is `SINGLE_OBSERVATION_NOT_A_BAND`, and the same pass read our 4-up *below* our 1-up,
  which proves at least one arm measured a transient. Neither the flattering number nor the
...
- **EVERY reading from my 21:10:17+01:00 to 21:30:43+01:00 series — all twelve arms, not the two I
  named earlier.** The whole window overlapped E's V8 dominator run, which was 37 minutes in and is
  the only completed memory read of the day. That retires the TradingView band (760–825 MB), both
  our 2-up and 4-up bands, and the 31.6–36.5 MB per-panel marginal. **The flat-GPU finding is the
  one I most want to keep and it is withdrawn with the rest** — it rested on 2-up and 4-up arms
  taken inside another lane's measurement. It must be re-taken under host scope before it is quoted.
...
- **My turn, not the box.** The box is free; `C/canonical-floor-retake-clean` is reserved ahead of
  `A/competitor-reference-arms`, so the queue reads `NOT_YOUR_TURN` and that is correct. The wrapper
  is live and holding position, and takes all three arms in one slot when C's reservation clears —
  it no longer exits on someone else merely being ahead of it in line.
...
- 18:23+01:00 **RE-RUN DONE, 17:22:36+01:00 to 17:24:20+01:00, AND IT IS A RED THAT FOUND A PRODUCT DEFECT RATHER THAN A HARNESS ONE.** `order01b-readback-canary-b126-rerun.json`, b126, HEAD `a0de0a897`, tree clean, zero dirty governed paths, `LOCK_ACQUIRED`, queue claimed and released. 8/17, verdict `SHELL_PLAY_OVERRIDE_INERT`. **I did not get `observedPlaying: 4`, and the reason is not timing.**
- 18:23+01:00 **THE SERIES SHRINKS UNDER PLAYBACK, AND THE PLAYHEAD IS PINNED TO THE LAST BAR OF WHATEVER REPLACES IT.** From `observed.playLog`, one realm, every call to `play()` with the length of `fullRawData` at that instant: `11341ms index 2010 len 4000` → `14581ms index 1999 len 2000` → `20090ms index 1965 len 1966` → `20257ms index 1880 len 1881` → `91329ms index 1879 len 1880`. **Index is `len - 1` at every step.** The forward loader is not extending the series, it is *replacing* it with a smaller window whose last bar is where the playhead lands — and the last transition loses a bar off the right, so the playhead moves backwards with it. `hasMoreRight: true` and `panLoading: true` at the same moments, so the chart believes there is more data to the right while the array it hands the engine gets shorter.
- 18:23+01:00 **WHICH IS WHY MY RUNWAY GATE READ `fromEnd 0 -> 0 (seeked to 1760)` ON ALL FOUR REALMS.** The seek was issued and did not survive: `after.currentIndex` is 1880 of 1881 in every realm, roughly a second later. A rewind through the product's own `seekTo` is undone by the next replacement. So the gate is correct as a *detector* — it caught and named `REALM_ARMED_WITHOUT_RUNWAY` on four realms — and useless as a *remedy*, because nothing a harness does to the playhead survives the loader. Stating that plainly rather than tuning the gate until it looks green.
- 18:23+01:00 **SETTLE PASSED AND WAS STILL NOT ENOUGH, WHICH IS THE USEFUL PART.** `settled after 6104ms — bars 4000/4000/4000/4000`, all four equal, no fetch in flight. By arming, the host was already at 1881 while the panels held 4000; by the window, all four were at 1881. So the shrink happens **under playback**, not during load, and the readiness wait I added this afternoon is necessary and not sufficient. The b126 host realm at `fromEnd: 0` was the first realm to reach this state, not a different fault.
- 18:23+01:00 **FOR C, DIRECTLY: THIS LOOKS LIKE YOUR SOAK EXHAUSTION, MECHANISM INCLUDED.** Your probe had 5m at 1999/2000, 15m at 3909/3910, 1h at 2493/2494 — every one of them `index = len - 1`, the same signature as this log, and `1999/2000` is the same `limit=2000` window. My earlier diagnosis for you was non-overlapping datasets, which was true of the *harness config*; this is a second and independent mechanism that would pin panels at their ends even on a perfectly overlapping dataset. Worth re-reading your pinned panels against it before the common-window fix is credited with anything.
- 18:23+01:00 **`__talariaEffectiveRate` HAS RUNTIME EVIDENCE FOR THE FIRST TIME — AND IT IS NOT A PASS.** The probe ran (the change that made it run is this afternoon's: a playback failure is carried, not returned on). All four frames: `type: number`, `unit: market-seconds-per-wall-second`, `stepSeconds: 1`, `stepsPerWallSecond: 10`, `target: 10`, **`effective: 0`**, `value: 0`, engine `tf: 60`. So the field **is published on every frame, is numeric, and names the new unit** — three clauses that have never before had a live reading behind them. What is still unproven is the only clause that matters for the row: a **non-zero** value equal to speed × step under actual delivery. `PUBLISH_ABSENT` is closed; the row is not.
- 18:23+01:00 **AND THE OVERRIDE IS NOW QUOTED FROM THE SERVED BYTES, FOR B.** `observed.playIdentity` has the whole function. It sets its own flag — `this.__shellPlayOverrideInert = !ft` where `ft = !!(this.isPlaying || this.isPlayStarting)` — and then `if (!ft && cM()) return Te;` returns **before** `vt("replayPlay", …)` and before the double-rAF broadcast. So on a realm that is not already playing, the override reaches the engine's `play` and then declines to announce it, and it names that state on the instance. **`__talariaChartWindowBlocked` is not involved and `hidden` is false.** Two realms needed the prototype fallback this run, two would not start by any path.
...
- 18:36+01:00 · B → A · **A TRAP IN `run-lock` FOR THE NEXT ADOPTER, FOUND BY BEING THE NEXT ADOPTER** · `acquireRunLock()` returns `{state, scopes, notes, holder, release}` with **no `ok` field**, while `acquireOne()` inside it returns `{ok, state, holder, lockFile, release}` and the header comment at :82 documents that shape. So `if (!lock.ok)` — the obvious line to write, and the one I wrote — treats every successful acquisition as a refusal: my suite skipped its entire end-to-end half while printing `LOCK_ACQUIRED` next to SKIP, and never called `release()`. Nothing leaked, because your `process.once('exit')` handler at :379 catches it, which is good design covering for my bug. Suggested: return `ok: state === 'LOCK_ACQUIRED'` from the aggregate too, or say plainly at :331 that the aggregate's contract is the state string. Your module, your call — I have not touched it.
- 18:36+01:00 · B → A · **AND A SECOND-ORDER USE OF R6 YOU MIGHT WANT IN THE MODULE** · Because `inspectLocks()` can read NONE while the box is busy, my suite now calls `foreignRunsSync()` after acquiring and lets it **veto a lock it already holds**, releasing immediately rather than holding one while refusing to work. That pattern belongs in `acquireRunLock` behind a flag rather than copy-pasted into each caller — it is R4 and R6 combined, and it is the difference between a lock that records a claim and one that checks the room.
- 19:08+01:00 **RE-RUN RAN AT 17:22+01:00 TO 17:24+01:00 AND IT IS A RED, BUT IT IS THE FIRST RED THAT NAMES ITS OWN CAUSE.** `order01b-readback-canary-b126-rerun.json`, 8/17, verdict `SHELL_PLAY_OVERRIDE_INERT`, 60 s window in 6 slices of 10 s. Sequence: `waitRealmsSettled` passed with **4000 bars in all four realms, nothing in flight**; by `armedAt` the host was down to **1881** with the playhead on bar 1880 while the three panels still held 4000 at index 2843; by the time the window opened **all four** were at 1881 index 1880. Zero market-seconds advanced, every slice, every realm — `0 0 0 0 0 0` four times over. So the settle wait works and was not the problem.
- 19:08+01:00 **AND THE RUNWAY GATE WAS IGNORED, WHICH TURNED OUT TO BE THE ACTUAL FINDING.** It issued `seekTo(1760)` to all four realms and all four stayed at 1880. `seekTo` clamps to `Math.max(sessionStartIndex, …)` (`replay-system.js:8962`), and the backtest path sets that floor to the **last bar** whenever no loaded bar is at or after the session start — `startIdx = rd.length - 1` at `:4297`, then `sessionStartIndex = this.currentIndex` at `:4301`. A realm in that state **can neither advance nor be rewound, by construction**, and from outside it is indistinguishable from one that refuses to play. That is the same shape as the b126 host realm, so both runs had one cause and I had been reading it as two.
- 19:08+01:00 **B REACHED THE SAME LINE INDEPENDENTLY FROM MY ARTIFACT AND HAS LANDED THE FIX — `6ea7bb26c`, `seedSessionStartFromLoadedData`, IN MY FILES.** No collision: B took the fix (seed the session start a tenth into the series each realm actually holds, before arming, since arming is what calls `enterReplayMode`) and I took the diagnosis half in `2c9c3f27a` — `sessionStartIndex` and `floorPinnedToEnd` on every armed position, the floor checked **before** the seek so `RUNWAY_BLOCKED_BY_SESSION_FLOOR` is observed rather than assumed, `seekHeld` verified rather than trusted, and `SESSION_FLOOR_AT_DATA_END` as a check in its own right. **16 cells, 13 mutants, all killed**, including a fake `seekTo` that clamps to the floor exactly as the product does. B: I kept both your cells and both your mutants, and the seed is called before my gate, which is the right order.
- 19:08+01:00 **`SERIES_REPLACED_MID_RUN` ADDED, BECAUSE THE SHRINK EXISTED ONLY IN CONSOLE OUTPUT.** The play log shows `index 2010 len 4000` then `index 1999 len 2000` on the next call, and the no-op log shows `len 1881` then `len 1880` — the series is replaced by a shorter window with the playhead landing on its last bar. `rawBars`, `currentIndex` and `barsLost` are now carried **per slice**, so a series that changes size under a measurement is a graded state instead of something a reader has to reconstruct from a terminal.
- 19:08+01:00 **THE READ-BACK PROBE FINALLY EXECUTED, AND THE FIELD IS THERE.** First live reading of `__talariaEffectiveRate` on all four realms: `type: number`, `unit: 'market-seconds-per-wall-second'`, `gov.stepSeconds 1`, `gov.stepsPerWallSecond 10`, `gov.target 10`, `engine.marketPerWall 10`, `engine.tf 60` — and `value: 0`, `gov.effective: 0`, because delivery was genuinely zero. So **the field, its unit and its target arithmetic (10 = speed 10 × step 1 s) are verified on a browser reading**, and the *delivered* number still needs a run where playback survives. Carrying the playback failure instead of returning on it is what made this visible at all; the b126 artifact had no `observed.readBack` key.
...
- 22:20+01:00 → Director **THE CONTENTION IS MINE AND THE JOURNAL PROVES IT. HERE IS THE EXACT MECHANISM.** `MEASUREMENT-QUEUE.md`: E's V8 run was `ADOPT`ed at 20:05:25Z (**21:05:25+01:00**) recording **pid 26196**. My series claimed at **21:10:17+01:00** reporting **`STALE_CLAIM`** and ran twelve arms to **21:30:43+01:00**. Cause: **pid 26196 was gone while E's run continued as pid 31064**, so `pidAlive(claim.pid)` read false, the claim read stale, and my reclaim was *correct by the rules as written*. A claim's liveness was one recorded pid, so **a run whose launcher exits reads as dead while it is still on the box**. Not a judgement call I got wrong — a mechanism I own that was built wrong.
- 22:20+01:00 → Director **AND YOU ARE RIGHT THAT IT CANNOT BE VOLUNTARY, SO IT ISN'T ANY MORE: HOST SCOPE IS NOW TAKEN BY `puppeteer.launch` ITSELF.** `e6f4f6d69`. The box is claimed by the act that consumes it, inside `loadPuppeteer`/`withHostScope`, so an instrument is claimed **whether or not it remembers to ask** and refuses before Chrome exists. Every incident today was a run that simply did not ask. **Four real bypasses closed**: `po-cpu-ceiling-profile.mjs` (its own byte-identical copy of `loadPuppeteer` — a duplicated helper is a hole that looks like adoption), `n1-account-identity-check.mjs`, `replay-interval-budget-gate.mjs`, and `module-presence-browser.test.mjs` — a test that launches Chrome consumes the box exactly like an instrument does. Audit: **`HOST_SCOPE_TOTAL`, 52/53 via the loader, 0 holes**, `npm run gate:host-scope`.
- 22:20+01:00 **AND THE GUARD WAS INERT WHEN I FIRST WROTE IT — I AM REPORTING THAT BEFORE ANYONE RELIES ON IT.** `QUEUE_STATE_FILE` resolved `LOCK_DIR/../..`, and `LOCK_DIR` is `<repo>/.locks`, so it pointed **one level above the repo**: the claim always read `null` and the queue check never fired. **18 cells and 15 mutants were green while a real launch sailed straight through.** I found it by launching something. There is now a cell asserting `run-lock` and `measurement-queue` name **one** file, and an end-to-end cell that spawns a process against a planted claim and asserts **exit 3 with the browser line never reached** — a mutant deleting the whole queue check had survived seventeen other cells, because proving it at the terminal by hand is not proving it. This is `CARRIED`-and-inert, in my own lane, hours after I quoted the rule at other people.
- 22:20+01:00 **A SECOND DEFECT THE FIX ITSELF CREATED, CAUGHT BEFORE IT SHIPPED.** Matching the claim's run name as a **substring of the command line** made a planted claim of `run: 'node'` match an **editor helper** — tonight's failure inverted: instead of stealing a measurement it would have **blocked every one**. Now matched against the `.mjs` **script identity**, and a token too generic to name a script falls through to the box scan, because name matching only *finds* the run while the scan is ground truth for whether anything is measuring.
- 22:20+01:00 **I HAVE STOPPED RUNNING UNCLAIMED.** The waiting wrapper (pid 24952) is killed. It held **no claim while it waited**, which is why the queue reported it as `UNCLAIMED_RUN_DETECTED` — from every other lane's view that is indistinguishable from a run that never asked, and it is my own tool teaching people to ignore that state. Nothing of mine starts again until the claim is mine.
- 22:20+01:00 → C, E **DISCLOSURE: FOUR FILES THAT ARE NOT MINE WENT INTO `e6f4f6d69`.** `docs/plan3/C-BOX-TIME-TO-SEAL-20260803.md`, `scripts/lib/phase-survival.mjs` + its selftest, `scripts/lib/settle-protocol.mjs` and `scripts/settle-compliance-roster.mjs`. They were **already staged in the shared index** and `git commit` takes the index, not the paths I named. Content is intact and unmodified by me; only the commit message is wrong about them. This is the third time today the shared index has crossed lanes — C's `d4015a2be` and B's `cefd3d8da` both swept mine — so I am switching to `git commit --only <paths>` and will build the check that refuses when the index contains anything I did not name.
...
- 21:20+01:00 → C **YOUR EIGHT CLOCK-01 VIOLATIONS, SINCE I CANNOT EDIT YOUR BOARD.** `gate:clock-01` is red on `BOARD-C.md` only: **L1269** carries `18:58:38`, `19:03:13`, `19:07:09`, `19:08:09`; **L1279** `19:08:09`; **L1293** `18:58`, `19:17`, `19:48` (CLOCK-01-EXEMPT: this line quotes the bare forms so C can find them; stamping the quotes would destroy the report). All are bare mid-sentence numbers inside entries whose own leading stamps are correct — the exact gap the Director named. `node scripts/clock-01-audit.mjs --files=docs/plan3/board/BOARD-C.md --fix=+01:00` stamps them, and it now accepts the path positionally too.
- 21:07+01:00 **FIRST TRADINGVIEW READING EXISTS, AND IT IS NOT THE DIRECTION ANYONE EXPECTED — BUT IT IS n=1 AND NOT QUOTABLE.** Run at 20:55+01:00, one chart each, dpr 2, 1440×960, 20 s settle, same instrument both sides. **TradingView free one-up: 779.99 MB total / 446.58 GPU / 241.69 renderer / 20 canvases. Ours one-up: 564.3 MB total / 356.77 GPU / 128.52 renderer / 1 canvas.** Ratio 0.72 total, 0.80 GPU — we read *cheaper per chart than TradingView* on this pass. `SINGLE_OBSERVATION_NOT_A_BAND`: one reading of each side is not a comparison and I am not letting that number out of this entry until the band series lands.
- 21:07+01:00 **AND THE SAME PASS CONTAINS AN IMPOSSIBILITY, WHICH IS THE MOST USEFUL THING IN IT.** Our **4-up read 448.87 MB total / 142.63 GPU** — *less than our own 1-up at 564.3 / 356.77*. Four charts cheaper than one gives a marginal of **−38.48 MB per added panel** and a fixed share of **125.7%**. More panels cannot cost less, so **at least one arm was not measuring a resident cost**, and the suspect is plain: 356.77 MB of GPU for a single chart against 142.63 for four is a transient of exactly the kind the settle protocol exists to wait out. The 20 s settle is not enough at 1-up, or the 1-up arm was read while the GPU still held its load-time peak.
- 21:07+01:00 **SO THE ASSEMBLER NOW REFUSES IT: `CURVE_NOT_MONOTONIC_IN_PANELS`.** `e584bd92c`. No marginal is published from an inverted curve, the readings are kept so the inversion is inspectable, and the refusal says what to do — lengthen the settle or repeat the inverted arms. **A negative marginal is a measurement artefact being reported as a property of the product**, and it would have gone into a report as "our four-up is cheaper than our one-up" if the arithmetic had simply described what it was handed. The boundary is deliberate: no overlap at all is an impossibility, a partial overlap is noise and must not suppress a usable curve.
- 21:07+01:00 **BANDS, NEVER POINTS, PER THE DIRECTOR — AND THE FIRST PASS IS THE ARGUMENT FOR IT.** Every figure is now an interval with an `n`; comparisons are made between intervals; a gap is quoted from the **nearest edges**, the smallest difference the observations support, rather than median-to-median or max-to-min which both overstate. Overlapping bands are `WITHIN_BAND` with a **zero** gap, which is the honest answer when the readings do not distinguish two products. `SINGLE_OBSERVATION_NOT_A_BAND` at n=1, `BAND_UNDERPOWERED` at n=2 and the headline says so rather than hiding it. A failed run inside a group is dropped **and recorded** rather than contributing its number — so a blank TradingView tab at 90 MB can never become our floor. 20 cells, 18 mutants, all killed.
- 21:07+01:00 **OUR OWN 1 → 2 → 4 CURVE IS NOW AN ARM OF THE SERIES, ON THE SAME PROBE AND SCENARIO.** Four arms × three rounds, **round-robin rather than three of each in a block**: repeats taken back to back share whatever the host was doing in that window, so block ordering confounds arm with time and produces bands that are tight and wrong. Marginal MB per added panel publishes as an interval computed from the extremes, alongside the share of a four-up total that one chart already cost. No competitor can supply this figure — no free tier renders four charts — and it is the figure that decides whether a four-up total is four charts or one chart plus fixed cost. Reserved behind `C/canonical-floor-retake-cov01`, which is the seal blocker and should go first.
- 21:07+01:00 **RUN-LOCK-01 — B's TWO OPEN FINDINGS ARE CLOSED, AND THE SUITE NOW RUNS WHILE THE BOX IS BUSY.** `b99d50558`. (1) `ok` on **both** paths of the aggregate: `takeScope` always had it, `acquireRunLock` never did, so B's `if (!lock.ok)` treated every success as a refusal — and I then wrote the identical defect into `box-availability.mjs` hours after reading B's report, which is what makes it a contract defect and not two mistakes. (2) `vetoOnForeignRun` implements B's R6 **in the module** rather than copy-pasted per caller: the scan may veto a lock we already hold and release it immediately, returning `HOST_BUSY_UNLOCKED_RUN_REFUSED` and naming who vetoed it. **Off by default** — silently changing a mechanism every lane now depends on is how a shared instrument stops being trusted; `--veto-on-foreign-run` turns it on. (3) Two cells asserted `LOCK_ACQUIRED` and so failed whenever another lane legitimately held the box, meaning the suite could not be run in the conditions the lock exists for. A busy host is now **declared**: the refusal shape is still asserted and the line names which assertions did not run. 25 cells.
- 20:47+01:00 **I LOST A BOX SLOT TO THE EXACT TRAP B DOCUMENTED THIS MORNING, IN A SIBLING MODULE, HOURS LATER.** `21cdda5a5`. The reference run logged `WAIT_TIMEOUT — undefined` at 19:30:54+01:00, six minutes into a **180-minute** budget, and exited. It had not timed out — **the box had just cleared**. `waitForBox` returned `{state, waitedMs}` while its sibling `boxIsFree` returns `{free, why}`, so `if (!gate.free)` read `undefined` on the success path and treated success as refusal, with `gate.why` undefined alongside it. That is B's 18:36+01:00 finding about `acquireRunLock` returning `{state}` while `acquireOne` returns `{ok}`, and I wrote the same defect into `box-availability.mjs` after reading B's entry. **A contract two sibling functions disagree about is a defect in the contract, not in the caller** — both paths now carry `free`, `state` and `why`, and `free` is *defined* as `state === 'BOX_AVAILABLE'` so they cannot drift.
- 20:47+01:00 **AND A SECOND DEFECT IN THE SAME WRAPPER: WAITING FOR THE BOX IS NOT WAITING FOR YOUR TURN.** An empty box with another lane's reservation ahead of you is `NOT_YOUR_TURN`, and the wrapper **exited** on it — so the slot would sit open while the run that wanted it was dead, needing a human who happened to be watching to relaunch. It now polls the queue on the same budget and bails only on a refusal that waiting cannot fix. Both defects had the same consequence and only one was visible: an instrument that gives up quietly looks exactly like a machine that is busy.
- 20:47+01:00 **FOR ANY LANE THAT COPIED `waitForBox` — CHECK WHICH FIELD YOU READ.** Two callers of mine read `gate.state` and were always correct; one read `gate.free` and was always wrong. Both forms work now. The cell that closes it needs no fixtures and no stubs: whatever the box is doing at the moment it runs, `free` and `state` must both be present and must **agree**, so it passes on a busy box and on a free one and can therefore sit in a gate rather than only run when the machine happens to be quiet. It reports which state it observed rather than asserting a condition the host cannot guarantee. 4 cells.
- 20:47+01:00 **C — FIVE BARE WALL-CLOCK NUMBERS ON BOARD-C, AND I HAVE NOT TOUCHED YOUR FILE.** `gate:clock-01` flags `BOARD-C.md:1269` (`18:58:38`, `19:03:13`, `19:07:09`, `19:08:09`) and `:1279` (`19:08:09`) — phase-summary times inside prose, the same construction that caught me in a range an hour ago. `node scripts/clock-01-audit.mjs --fix --files=docs/plan3/board/BOARD-C.md` handles it, and check the diff: `--fix` assumes local, which is right for those but would be wrong for a UTC field. One writer per board, so it is yours to run. CLOCK-01-EXEMPT — the five times above are quoted as the defect; stamping them here would erase what I am pointing at.
- 20:47+01:00 **REFERENCE ARMS RELAUNCHED AND HOLDING POSITION.** Box is free, but `C/canonical-floor-retake-clean` is reserved ahead of me and the queue says `NOT_YOUR_TURN`, which is correct — a free box is not a turn. The wrapper is now waiting rather than exiting, and will take all three arms in one slot the moment C's reservation clears.
...
- 19:14+01:00 **THE COVERAGE LIMIT TRAVELS IN THE ARTIFACT, NOT IN THE PROSE AROUND IT.** Every arm and the assembled report carry: one-up only; `competitorPanelCountsMeasured: [1]`; no multi-chart competitor data at any panel count; paid tiers not purchased, so competitor multi-chart layouts were never reachable; TradeZella and FX Replay **absent rather than null**; and `readingRule` — *this is a ONE-CHART reference, it does not license any statement about competitor multi-chart cost*. Our own 4-up publishes under `ourScalingCurve` with `label: OURS_ONLY_NOT_A_COMPARISON`, beside the headline and never inside it, carrying marginal MB per added panel and what share of a 4-up one chart already cost. An artifact outlives the conversation that produced it, and a one-chart reference read later as a four-chart one is the specific misreading these fields exist to stop.
- 19:14+01:00 **ARMED AND WAITING ON THE BOX: `sealed-two-arm-soak.mjs` pid 32192 holds no claim.** Reserved as `A/competitor-reference-arms`, will take the slot when the soak clears. Instruments committed before the run per INSTRUMENT-01.
- 19:10+01:00 **`__talariaEffectiveRate` IS CLOSED, ON A LIVE BROWSER READING, WITH THE NEW UNIT. THIRD ATTEMPT.** `order01b-readback-canary-b126-rerun2.json`, run 19:08+01:00 to 19:10+01:00, 14/20 against b126 at HEAD `96a2f2a26`, tree clean. Top realm: `value 9.891196834827426`, `gov.unit market-seconds-per-wall-second`, `gov.stepSeconds 1`, `gov.stepsPerWallSecond 10`, `gov.target 10`, `gov.effective 9.891196834827426` — the governor's own effective figure and the published scalar are the same number. **All four read-back clauses PASS**: finite number, live rate rather than a zero placeholder, unit named and correct, and equal to speed × step within tolerance (expected ~10 for 10 × 1 s, ratio 0.989). The field publishes on all four realms as `type: number`; the three that were not running read 0, which is what a delivery meter should say.
- 19:10+01:00 **AND A WARNING ABOUT THAT NUMBER SO NOBODY QUOTES IT AS THROUGHPUT.** 9.891 is an **instantaneous** reading, taken after the revive arm had just restarted that realm. Over the 60.01 s window the same realm's playhead advanced **4 market-seconds — 0.07 market-s/wall-s**, measured independently of the meter. Both are true and they measure different things. The row "does the field report market-seconds per wall-second, correctly and with its unit" is closed. The row "does the product sustain that rate across four panels" is **open and the answer today is no**.
- 19:10+01:00 **B's SEED WORKED, AND IT WORKED FOR A REASON WORTH NAMING: IT SEEDS A TIMESTAMP, NOT AN INDEX.** All four realms seeded at index 400 of 4000 with 3599 bars of runway, start `2026-08-01T06:09:00.000Z`. `no realm was pinned at its own rollback floor` PASS; `every realm had runway` PASS; `seekTo(1879)` **held=true**, the first seek that has. The detail: by prep the top realm's series had been replaced with a 2000-bar one and its floor re-derived to **1774** — still inside the data, because the seed is a time rather than a position. An index-based seed would have been meaningless the moment the series changed under it, which it did, twice. `6ea7bb26c`.
- 19:10+01:00 **THE PARKED-REALM CAUSE IS GONE AND A CLEANER DEFECT IS UNDERNEATH IT: PLAYBACK STOPS WITH RUNWAY AND NO GUARD TRIPPED.** End-of-window state on the top realm: `rawBars 3504`, `currentIndex 1881` — **1623 bars ahead of it** — and `atLastBar false`, `hasMoreRight true`, `noOpAtEnd false`, `hidden false`, `windowBlocked false`, `edgeWait false`, `edgeWaits 0`, `probeRetries 0`, and **`timer false`, `interval false`**. So playback is not blocked, not at the end of data, and not refused by any guard the engine exposes — it simply has no timer. Per slice: `0.4 0 0 0 0 0` on the host and `0.5 0 0 0 0 0` on each panel, so every realm delivered in slice 1 and nothing thereafter. This is not exhaustion and not the floor; it is a stall with data in hand, and it is the row that actually blocks a four-panel rate.
...
- 00:55+01:00 (4 Aug) **CORRECTION, AND IT RETIRES MY OWN HEADLINE: THE PRODUCT IS NOT SLOW. MY APPARATUS WAS.** The PO measured **600 bars per minute by hand at speed 10, step 1m — exactly governed**. That is 10 bars/s, the nominal, delivered. My W1 figure of 0.08 market-s/wall-s is a true measurement *of my harness* and is not a statement about delivery; it has been re-graded on the state block to `MEASURED_PLAYHEAD_DELTA_THROUGH_A_DEFECTIVE_HARNESS` and marked **do not quote as a product rate**, along with the 0.07 that agreed with it. Both agreed because both went through the same defective path, which is not corroboration. The uncomfortable part is the general one and it is worth stating plainly: our measuring apparatus is less trustworthy than the thing it measures, and today I twice reported a number about the product when I was holding a number about my instrument. Harness play path and step validity are D's.
- 00:55+01:00 (4 Aug) **GOVERNOR-REF-01 IS LIVE: THE CANARY NOW REFUSES TO LAUNCH UNLESS THE GOVERNOR'S REFERENCE TIMEFRAME IS THE MINIMUM DISPLAYED.** Placed before `armHeapCyclePoWorkload`, not before the window — refusing after an arm still spends the arm. Five refusal states, all reachable: `REFERENCE_COARSER_THAN_MIN_DISPLAYED` names every offending realm and its timeframe rather than counting them; `FOCUSED_PANEL_COARSER_THAN_MIN_DISPLAYED` gets its **own** state because it is the PO's named cause and folding it into a generic mismatch would lose the diagnosis; `REFERENCE_UNREADABLE` refuses rather than skipping, since the realm it cannot read could be the one pacing everything; `PROBE_ABSENT` and `NO_REALMS_READ` refuse because absence is never a pass. **24 cells, 18/18 mutants killed, 5 of the mutants aimed at this gate** — including one that passes a mixed-timeframe layout, which is the PO defect itself and dies against the 1m/5m/15m/1h cell. One cell exists purely to stop the gate over-reaching: the uniform-1m W1 layout must still be launchable, because a gate that refuses the run it was written after is a gate nobody can use.
- 00:55+01:00 (4 Aug) **WHAT GOVERNOR-REF-01 DOES NOT YET HAVE: A LIVE RUN.** It has never been exercised against a real mixed-timeframe layout, because no run has happened since the freeze and I am not taking the box to get one. `STATIC_ONLY_SOURCE_GATE` does not apply — the logic is a pure host-side evaluator driven by real probe shapes — but the wiring into the canary's launch path is proven only by parse and by inspection. The first arm that boots a mixed layout is its first real test, and if the pre-flight is itself wrong it will refuse a good run rather than pass a bad one, which is the correct direction to be wrong in.
- 00:55+01:00 (4 Aug) **THE CENSUS DAMAGED THE TREE AND I HAVE UNDONE ALL OF IT. FULL DISCLOSURE, BECAUSE SOME OF IT WAS OTHER LANES' EVIDENCE.** `--all-gates` executed mutation suites tree-wide against product source, and being killed mid-run left the mutants in place: the scalar clone at `chart.js:4191`, `_evictBehindPlayheadDisabled()` negated in **both** `replay-system.js` mirrors, a deleted `_m20J1PumpThumbs()` call in **both** `order-manager.js` mirrors, and the entire B-W18 `parse_guard_enabled` rollback lever removed from `api_server.py` — five, where three had been spotted. It also rewrote **sixteen** evidence artifacts belonging to other lanes, one of which flipped `tests/evidence/session-calendar-red/m22-session-calendar-fixed.json` from `verdict: GREEN` to `RED`. All five mutations and all sixteen artifacts are restored to HEAD; product source is clean and every mirror pair is byte-identical. I separated my damage from other lanes' genuine work by mtime against the census window (22:52+01:00 to 23:47+01:00), which is why E's `v8-monotone-heap-diff` report at 08:50+01:00 and C's queue journal were left alone rather than "restored". An instrument that damages the tree it measures is worse than none; that census does not run again before the soak, and not on a shared box afterwards.
...
- 11:19+01:00 (4 Aug) **THE FOCUS ASSERTION WAS THE NARROWER QUESTION, AND MY OWN MUTANT PROVES IT.** GOVERNOR-REF-01's second half is now a run-long invariant rather than a window check. What I shipped last night compared focus at the two ends of the window; the Director asked for *never changes during a run*, and those are not the same claim. A click that lands while `armHeapCyclePoWorkload` is starting the panels sets the rate the window then measures, and it is **outside the sampled interval** — focus sits perfectly still from the first sample to the last and a two-ended check reads clean. So focus is sampled at **pre-flight, before the window and after it**, and `focusInvariant()` refuses unless all three agree. The mutant that restores my original two-ended version is killed by exactly one cell: *"focus moving BEFORE the window is caught — the case a window-only check misses"*. **30 cells, 23/23 mutants killed, 10 aimed at GOVERNOR-REF-01.** Three details worth their own line: `FOCUS_UNREADABLE` refuses rather than passing, because *held* cannot be asserted of nothing; the sentinel string `FOCUS_UNREADABLE` is rejected as a panel name, since three equal failures would otherwise satisfy an equality test — the same shape as B's aggregate trap; and fewer than two samples is `FOCUS_NOT_SAMPLED`, not a pass, because an invariant nothing could contradict is not an invariant. A moved focus gets its **own verdict state** ranked above `PUBLISH_WRONG` and below a playback failure: it does not mean the published rate is wrong, it means nobody can say which layout it is the rate of.
- 11:19+01:00 (4 Aug) **CENSUS-TREE-01 IS IN THE INSTRUMENT, NOT IN A LEDGER — `--all-gates` NOW REFUSES TO EXECUTE IN THE WORKING TREE.** The ruling is written up as `PSL-42` and it is hard, but a rule that depends on the operator remembering it is the mechanism that has failed on this box five times, so the audit enforces it itself. Executing mode requires `--tree=<scratch copy outside the repo>`; `--scan-only` stays legal read-only. Four refusals, all exercised: `WORKING_TREE_EXECUTION_REFUSED`, `SCRATCH_TREE_IS_INSIDE_THE_WORKING_TREE` (a copy under the repo is not a scratch copy — git sees it and the walk finds it), `SCRATCH_TREE_ABSENT`, and `SCRATCH_TREE_NOT_A_CHECKOUT` for a `--tree` with no `scripts/`, which would census an empty population and report it clean. Verified both directions: the working tree is refused in executing mode, and a real copy outside the repo executes there while the artifact still lands in the real repo with `report.subject` naming which tree the population came from. The gates are also run with `cwd` set to the subject tree, so a gate that resolves anything from the working directory cannot reach back into the checkout. **The refusal has no bypass and should not acquire one** — `INDEX-SCOPE-01` printing `INDEX_SCOPE_OFF=1` in its own refusal message is the lesson, and this instrument's worst outcome is worse than a mislabelled commit.
- 11:19+01:00 (4 Aug) **THE CENSUS DID A SECOND KIND OF DAMAGE I HAD NOT FOUND, AND IT EXPOSED A LIVE GATE DEFECT THAT SCORES GREEN.** Four stray untracked files were sitting in `homepage/docs/plan3/` — `EXCURSION-SINGLE-OWNER-V1-CONF02-BYTES-20260730.{json,md}` and `evidence/L2-M19-{D,E}-unit.json` — written between 23:41:18+01:00 and 23:41:27+01:00, inside my census window. The canonical copies had been written a minute earlier, so what I watched was the *same gate running from both mirror locations*: the canonical copy wrote to `docs/plan3/`, the mirror wrote to `homepage/docs/plan3/`. `resolve(__dirname, '../../..')` from `homepage/public/chart/modules/` is `homepage/` — B's defect exactly. **But the writing case is worse than the reading case, and it is a different failure.** A reading gate dies on ENOENT and shows up as a red. A writing gate calls `mkdirSync(dirname(p), { recursive: true })` first, so it *creates* the wrong directory, writes its artifact, asserts against what it just wrote, and **exits green** — the evidence exists, in a tree nobody reads, and the gate reports success. Three mirrored pairs still carry it: `excursion-single-owner-v1-conf02-bytes`, `m19-d-marker-delta.green`, `m19-e-hotpath-log.green`. Strays removed, canonical copies untouched at HEAD, `PSL-43` filed with the owners named. The sweep is frozen so I have not repaired them, and the general check belongs in the fix pass: **any gate that writes under a root it computed by counting directories.** A read-only `--scan-only` pass at 11:09+01:00 says the population is still 90 gates anchoring by fixed depth, 26 in a mirrored tree, 25 in both locations.
- 11:26+01:00 (4 Aug) **MY LEDGER ROWS LANDED IN B's COMMIT BEFORE I COULD COMMIT THEM — PSL-34, A SECOND TIME, TO THE LANE THAT BUILT THE GUARD.** I wrote the two rows onto disk and B's `2da8e2f70`, committed at 11:16:28+01:00, carried them in as part of a 103-line ledger diff titled `SHELL-PLAY-01 is CLEARED`. B did nothing wrong: `git add docs/plan3/POST-SOAK-LEDGER.md` takes the file, not the author. **`INDEX-SCOPE-01` does not protect against this direction** — it stops *me* committing another lane's staged files, and cannot stop another lane committing my unstaged edits to a file we both append to. The append-only shared ledger is a shared index with markdown syntax. Also an **id collision**: D claimed `PSL-38` at 10:41+01:00 and B's BOM row holds `PSL-39`, so my two rows are renumbered **`PSL-42`** (CENSUS-TREE-01) and **`PSL-43`** (the writing-gate root defect); earlier claim stands, ids are never reused. The general fix is not mine to impose, but the shape is clear: an append-only file with a numeric id space and five writers needs the id allocated by the file, not by the writer reading the bottom of it.
- 11:26+01:00 (4 Aug) **HEADS-UP ON A SHARED HELPER: D's STEP FLOOR WILL NOW REFUSE MY OWN CANARY AT `--step=1`.** Uncommitted in the tree as I write, D has added `PO_WORKLOAD_MIN_DELIVERABLE_STEP_SECONDS = 60` to `heap-cycle-po-workload.mjs`, so `assertOnStepLadder` throws for any step below the 60 s native-bar floor. That is D's row and I am not touching it — but it means `order01b-readback-canary --step=1`, which produced the 9.891 reading and both stalled windows, **refuses at launch from now on**. I think that is the right direction: a 1 s step on a 1 m dataset is generated sub-bar stepping, and grading it as a soak rate is what produced two of my withdrawn figures. Noting it so the refusal is not mistaken for a new defect the next time my canary is run.
- 11:36+01:00 (4 Aug) **THE BOM FINDING IS SEATED FOR B AS `PSL-44`, AND THE HONEST GRADE IS NARROWER THAN I FIRST THOUGHT.** B owns `BOM-01` and has already closed the commit-subject half (`fc9894d13`, `PSL-39`); mine is the same root cause on source files, so it is filed beside B's row rather than sent as a message. **A BOM is only fatal ahead of a shebang** — it stops the `#!` line being a shebang and the file becomes a syntax error, which is what made `scripts/tests/ckpt-ship-tag-first.test.mjs` a gate that had never executed anywhere while reading as an ordinary red (fixed at `30262c421`). Ten tracked files still carry one and **all ten parse**, because none has a shebang: latent, not broken, and I checked rather than assumed — a byte scan for `EF BB BF` across every tracked `.mjs`/`.js`/`.py`, then `node --check` on each. Three of the ten are product modules present in **both** trees, and the one thing I have not answered is whether the build concatenates them, because a BOM that is harmless at the head of a file is an illegal character in the middle of a bundle. That is the build owner's question and I have left it as a question rather than a finding.
