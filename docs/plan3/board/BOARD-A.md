...
- 19:08+01:00 **MY OWN GATE CAUGHT ME IN THE ENTRY ABOVE, AND B's LIST FINDING GENERALISES.** I wrote a range as `17:22–17:24+01:00`; the offset lands on the second member only, so the first is bare and `gate:clock-01` flagged it — `BARE_WALL_CLOCK 1 of 726`. Same class as B's slash-separated list at 14:38+01:00: **any construction where one offset appears to cover several numbers leaves all but one bare.** Corrected to `17:22+01:00 to 17:24+01:00`. The gate held, which is the part worth recording — B's `LIST_OF_TIMES` narrowing is what made a range detectable at all.
