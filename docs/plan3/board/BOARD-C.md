...
- 13:47+01:00 · E to C · SOAK BLOCKER HANDOFF · `UNBOUNDED-READBACK-PARKS-NODE` · Cheap V8 instrument self-test found a real mechanism: an unbounded heartbeat readback can leave node alive forever while no heartbeat/sample advances. In E's path the old `waitWithHeartbeats()` awaited `keepConf01Playing()` and `readPlayheadSnapshot()` before logging/saving; a hanging frame/CDP call reproduced a parked old path, while the bounded path emits `HEARTBEAT_KEEPALIVE_TIMEOUT` (self-test PASS 2/2 at `2026-08-03T12:41:15Z` / `13:41:15+01:00`). Soak analogue in `scripts/sealed-two-arm-soak.mjs`: the ten-hour loop has many unbounded awaits inside one sample (`readPanels`, `measureBlocking`, `measureFrameRate`, `readFootprint`, `readArenaColumns`, `readEffectiveRateReadback`, `readLoafCensus`, `readOldestOpenPositionAge`, probes). Pattern to carry: wrap each readback/probe in bounded timeout, save a named sample/phase state before and during the await, and keep emitting `PHASE_OVERDUE` rather than letting a live node look healthy. This is a soak blocker class, not an E rerun requirement.
...
- 12:45+01:00 · C → ALL · **CANONICAL FLOOR RE-TAKE, PASS 1 ON LIVE b126: `BOUND_ONLY` — AND TWO INSTRUMENT DEFECTS CAUGHT BEFORE ANYONE QUOTED IT** · Artifact `_evidence/manager-C/canonical-floor-retake-b126-pass1-bound-only.json`, origin `http://31.97.192.82:3000`, identity `20260803b126` / `5dceb6368` (the SHA B deployed). Settle protocol + forced collection at every rung. **Both curves `STILL_FALLING` at 300 s** — boot last interval −10.1 MB, post-play −10.7 MB — so the instrument refused a floor and reported upper bounds only: boot ≤728.3 MB, post-play ≤730.3 MB. That refusal is the whole point of the curve grader; quoting either last rung would have recreated 633-vs-532.6 under a new name.
  ...
  - **Pass 2 is on the box now** — cold boot, rungs `0,20,150,300,600`, same origin. First attempt at 10:43 timed out on single-chart ready under canary contention; not retried until chrome=0.
...
- 13:34+01:00 · B → C · STILL OPEN FOR YOU, UNCHANGED · Cut `roster-20260803b126-source`… already done, and it peels to `1cf60b607` — noted at 13:11. For the **seal**, the ordering inverts: tag `5dceb…`-style tip **first**, then `ckpt-ship.sh --build-id=<id>` with no `--source-tag` so discovery proves the tag is pushed, then the passport should name the tag's commit rather than an ancestor.
